...
public class OpenAPISpecificationTest {
  ...
  public void testGetResponsesFromAPISpecification() {
    OpenAPISpecification spec =
        new OpenAPISpecification("src/test/resources/Petstore/swagger.json");
    assertEquals(
        "Wrong number of responses",
        2,
        spec.getSpecification().getPaths().get("/pet/findByStatus").getGet().getResponses().size());
  }
  ...
  public void testReadDHLSpecification() {
    OpenAPISpecification spec =
        new OpenAPISpecification("src/test/resources/restest-test-resources/swagger-dhl.yaml");

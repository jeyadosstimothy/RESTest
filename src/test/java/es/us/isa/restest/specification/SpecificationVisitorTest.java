...
public class SpecificationVisitorTest {
  ...
  public void shouldFindParameter() {
    ...
    assertEquals(
        "The parameter's name should be 'offset', but its name is" + param.getName(),
        "offset",
        param.getName());
    assertEquals(
        "'offset' should be a query parameter, but it is a " + param.getIn() + " parameter",
        "query",
        param.getIn());
    assertEquals(
        "'offset' should be an integer. Parameter type: " + param.getType(),
        "integer",
        param.getType());
  ...
  public void shouldGetRequiredParametersGetOperation() {
    ...
    MatcherAssert.assertThat(
        required.stream().map(OpenAPIParameter::getName).collect(Collectors.toList()),
  ...
  public void shouldGetParametersSubjectToInvalidValueChangeTravelAPI() {
    ...
    MatcherAssert.assertThat(
        parameters.stream().map(OpenAPIParameter::getName).collect(Collectors.toList()),
        contains("isAdmin", "maxPriceAirbnb", "includeTripsWithUnsetAirbnbPrice", "sort"));
  }
  ...
  public void hasDependenciesTest() {
    assertTrue(
        "The operation GET /trips/user should have dependencies",
        hasDependencies(travelApiOas.getSpecification().getPaths().get("/trips/user").getGet()));
    assertFalse(
        "The operation GET /trips shouldn't have dependencies",
        hasDependencies(travelApiOas.getSpecification().getPaths().get("/trips").getGet()));
    assertFalse(
        "The operation DELETE /trips/user shouldn't have dependencies",
        hasDependencies(travelApiOas.getSpecification().getPaths().get("/trips/user").getDelete()));
  }
}

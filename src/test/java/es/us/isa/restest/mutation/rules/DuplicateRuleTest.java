...
public class DuplicateRuleTest {
  ...
  public void applyDuplicateRuleCommentsObjectWithoutPropertiesTest() {
    OpenAPISpecification spec =
        new OpenAPISpecification("src/test/resources/Comments/swagger_forTestSuite7.yaml");
    Schema originalSchema =
        spec.getSpecification()
            .getPaths()
            .get("/comments")
  ...
  private boolean checkMutationIsValid(Schema originalSchema, Schema mutatedSchema) {
    ...
    if ("array".equals(mutatedSchema.getType())) {
      valid =
          checkMutationIsValid(
              ((ArraySchema) originalSchema).getItems(), ((ArraySchema) mutatedSchema).getItems());
        ...
        if (entry.getKey().endsWith("-duplicated")) {
          ...
          valid =
              originalSchema.getProperties().containsKey(propertyName)
                  && ((Schema) originalSchema.getProperties().get(propertyName))
                      .getType()
                      .equals(entry.getValue().getType());
        } else if (entry.getValue().getType().equals("object")) {
          valid =
              checkMutationIsValid(
                  (Schema) originalSchema.getProperties().get(entry.getKey()), entry.getValue());
        }
        if (valid) {
          break;
        }

  ...
  public void mutationAppliedTest() {
    ...
    OpenAPIParameter paramToMutate =
        new OpenAPIParameter(
            spec.getSpecification().getPaths().get("/gallery").getPut().getParameters().stream()
                .filter(p -> p.getName().equals("rarity"))
                .findFirst()
                .get());

    assertTrue(
        "The test case should be mutated",
        NumberToInvalid.mutate(tc, paramToMutate)
            .matches("Changed value of number parameter rarity from '.*' to .* '.*'"));
    assertFalse(
        "The value should not be a number",
        StringUtils.isNumeric(tc.getParameterValue(paramToMutate)));

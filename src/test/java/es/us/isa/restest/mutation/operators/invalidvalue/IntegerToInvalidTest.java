...
public class IntegerToInvalidTest {
    ...
    OpenAPIParameter paramToMutate =
        new OpenAPIParameter(
            spec.getSpecification().getPaths().get("/trips/user").getGet().getParameters().stream()
                .filter(p -> p.getName().equals("maxPriceAirbnb"))
                .findFirst()
                .get());

    assertTrue(
        "The test case should be mutated",
        IntegerToInvalid.mutate(tc, paramToMutate)
            .matches("Changed value of integer parameter maxPriceAirbnb from '.*' to .* '.*'"));
    try {
  ...
  public void integerMinMaxMutationTest() {
    OpenAPISpecification spec =
        new OpenAPISpecification("src/test/resources/AmadeusHotel/swagger.yaml");
    TestCase tc =
        new TestCase(
            "dfgsdfg",
            true,
    ...
    OpenAPIParameter paramToMutate =
        new OpenAPIParameter(
            spec
                ...
                .getParameters()
                .stream()
                .filter(p -> p.getName().equals("page[limit]"))
                .findFirst()
                .get());

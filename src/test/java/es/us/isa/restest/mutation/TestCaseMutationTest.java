...
public class TestCaseMutationTest {
  ...
  public void testCaseMutationTest() {
    ...
    assertNotEquals(
        ...
        "",
        TestCaseMutation.mutate(tc, spec.getSpecification().getPaths().get("/comments").getGet()));
  ...
  public void testCaseMutationImpossibleTest() {
    ...
    assertEquals(
        "The test case should NOT be mutated",
        "",
        TestCaseMutation.mutate(
            tc, spec.getSpecification().getPaths().get("/comments/{id}").getGet()));
  ...
  public void removeRequiredParameterTest() {
    ...
    assertNotEquals(
        "The test case should be mutated",
        "",
        RemoveRequiredParameter.mutate(
            tc, spec.getSpecification().getPaths().get("/comments").getPost()));
    assertNotEquals("The two test cases should be different", tc, oldTc);
  }
  ...
  public void removeRequiredParameterNotPossibleTest() {
    ...
    assertEquals(
        ...
        "",
        RemoveRequiredParameter.mutate(
            tc, spec.getSpecification().getPaths().get("/comments").getGet()));
    assertEquals("Both test cases should be equal", tc, oldTc);
  }
  ...
  public void invalidParameterValueTest() {
    OpenAPISpecification spec =
        new OpenAPISpecification("src/test/resources/Comments/swagger_forTestSuite.yaml");
    TestCase tc = new TestCase("dfgsdfg", true, "getComment", "/comments/{id}", HttpMethod.GET);
    tc.addPathParameter("id", "c1");
    ...
    assertNotEquals(
        "The test case should be mutated",
        "",
        InvalidParameterValue.mutate(
            tc, spec.getSpecification().getPaths().get("/comments/{id}").getGet()));
    assertTrue(
        "The length of the mutated 'id' parameter should be greater than 4",
        tc.getPathParameters().get("id").length() > 4);
    assertNotEquals("The two test cases should be different", tc, oldTc);
  }
  ...
  public void invalidParameterValueNotPossibleTest() {
    ...
    assertEquals(
        ...
        "",
        InvalidParameterValue.mutate(
            tc, spec.getSpecification().getPaths().get("/comments").getPut()));
    assertEquals("Both test cases should be equal", tc, oldTc);
  }
}

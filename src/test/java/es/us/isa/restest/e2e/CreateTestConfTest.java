...
public class CreateTestConfTest {
  ...
  public void testCreateTestConfWithArgs() {
    String[] args = {
      "src/test/resources/BigOven/spec.yaml",
      "/article/{term}:GET,PUT",
      "/grocerylist/item/{guid}:DELETE",
      "/recipe/review/{reviewId}/replies:ALL"
    };
    CreateTestConf.main(args);
  ...
  private void check() throws NoSuchFieldException, IllegalAccessException {
    ...
    if (!openApiSpecPath.equals("src/test/resources/Folder/openapi.yaml"))
      fail(
          "The CreateTestConf class should have 'src/test/resources/Folder/openapi.yaml' as the"
              + " default Swagger path.\n"
              + "This is to avoid testConf files being changed inadvertently when running the test"
              + " suite.\n"
              + "You may have changed it and forgot to set it back to its default value. Please, do"
              + " so.");
  }
}

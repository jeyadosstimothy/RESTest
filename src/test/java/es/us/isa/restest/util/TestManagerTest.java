...
public class TestManagerTest {
  ...
  public void testGetTestResults() {
    ...
    assertEquals(
        ...
        testResults.get(4).getResponseBody());
  }
  ...
  public void testGetTestCases() {
    ...
    for (TestCase tc : testCases) {
      if (testCases.indexOf(tc) != 9 && testCases.indexOf(tc) != 10)
        ...
        assertEquals(
            ...
            "{\n"
                + " \"error\": {\n"
                + "   \"message\": \"Received both percent_off and amount_off parameters. Please"
                + " pass in only one.\",\n"
      ...
      assertEquals(
          "The content type should be 'application/json'", "application/json", tc.getInputFormat());
    ...
    assertEquals(
        "The query parameter does not match",
        "outwear",
        testCases.get(6).getQueryParameters().get("proximity"));
  }
  ...
  public void testGetLastTestResult() {
    ...
    assertEquals(
        "The id does not match", "GETversionlocationsmarkersformatTest_t8f5p4k4yijn", tr.getId());
  ...
  public void testGetRangeTestResults1() {
    ...
    assertEquals(
        "The ID of the first TR does not match",
        "GETversionlocationsformatTest_1ies6plrqnv50",
        testResults.get(0).getId());
  }
  ...
  public void testGetRangeTestResults2() {
    ...
    assertEquals(
        "The ID of the first TR does not match",
        "GETversionlocationsformatTest_1ies6plrqnv50",
        testResults.get(testResults.size() - 1).getId());
  }
}

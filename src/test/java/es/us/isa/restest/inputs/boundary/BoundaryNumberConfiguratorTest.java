...
public class BoundaryNumberConfiguratorTest {
  ...
  public void testDefaultReturnedValues() {
    ...
    assertEquals(
        "Wrong next value", Integer.valueOf(Integer.MIN_VALUE).longValue(), iterator.next());
    assertEquals(
        "Wrong next value", Integer.valueOf(Integer.MIN_VALUE).longValue() - 1, iterator.next());
    assertEquals(
        "Wrong next value", Integer.valueOf(Integer.MIN_VALUE).longValue() + 1, iterator.next());
    assertEquals(
        "Wrong next value", Integer.valueOf(Integer.MAX_VALUE).longValue(), iterator.next());
    assertEquals(
        "Wrong next value", Integer.valueOf(Integer.MAX_VALUE).longValue() - 1, iterator.next());
    assertEquals(
        "Wrong next value", Integer.valueOf(Integer.MAX_VALUE).longValue() + 1, iterator.next());
    assertEquals(
        "Wrong next value",
        Double.valueOf(
                Math.ceil(Integer.valueOf(Integer.MIN_VALUE + Integer.MAX_VALUE).doubleValue() / 2))
            .longValue(),
        iterator.next());
  }

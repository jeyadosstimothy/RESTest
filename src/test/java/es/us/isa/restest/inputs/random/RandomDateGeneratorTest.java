...
public class RandomDateGeneratorTest {
  ...
  public void testRandomDateGeneration() {
    RandomDateGenerator gen =
        (RandomDateGenerator) TestDataGeneratorFactory.createTestDataGenerator(generator);
    for (int i = 0; i < 100; i++) {
  ...
  public void testRandomDaysBasedBoundedDateGeneration() throws ParseException {
    ...
    for (int i = 0; i < 100; i++) {
      ...
      assertTrue("Not a date", value instanceof Date);
      assertTrue(
          "Out of range", value.after(new DateTime(new Date()).plusDays(startDays).toDate()));

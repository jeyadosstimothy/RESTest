...
public class BoundaryStringConfiguratorTest {
  ...
  public void testConstructor() {
    ...
    assertTrue(
        "includeEmptyString property should be true by default",
        boundStrConf.getIncludeEmptyString());
    assertTrue(
        "includeNullCharacter property should be true by default",
        boundStrConf.getIncludeNullCharacter());
  }
  ...
  public void testValuesIterator() {
    ...
    gen.getGenParameters()
        .addAll(
            Arrays.asList(
                minLengthParam, maxLength, delta, includeEmptyString, includeNullCharacter));
    ...
    assertEquals(
        "Wrong string length for next value", 4, boundStrIter.nextValueAsString().length());
    assertEquals(
        "Wrong string length for next value", 6, boundStrIter.nextValueAsString().length());
    assertEquals(
        "Wrong string length for next value", 1024, boundStrIter.nextValueAsString().length());
    assertEquals(
        "Wrong string length for next value", 1026, boundStrIter.nextValueAsString().length());
    assertEquals(
        "Wrong string length for next value", 514, boundStrIter.nextValueAsString().length());
    assertEquals(
        "Wrong string length for next value", 4, boundStrIter.nextValueAsString().length());
    assertEquals(
        "Wrong string length for next value", 6, boundStrIter.nextValueAsString().length());
    ...
    assertEquals(
        "Wrong string length for next value", 514, boundStrIter.nextValueAsString().length());
    assertEquals(
        "Wrong string length for next value", 2, boundStrIter.nextValueAsString().length());
    assertEquals(
        "Wrong string length for next value", 2, boundStrIter.nextValueAsString().length());
    assertEquals(
        "Wrong string length for next value", 1022, boundStrIter.nextValueAsString().length());
    assertEquals(
        "Wrong string length for next value", 1022, boundStrIter.nextValueAsString().length());
  }
  ...
  public void testValuesRandomIterator() {
    ...
    for (int i = 0; i < 7; i++) {
      ...
      assertTrue(
          "Wrong next value obtained: " + value + ", length: " + value.length(),
          values.contains(value) || lengths.contains(value.length()));
    }
  }
}

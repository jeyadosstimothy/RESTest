...
public class CoverageGathererTest {
  ...
  public void coverageGathererTest() {
    ...
    assertEquals(
        "There should be 9 criterion types", 9, covGath.getCoverageCriterionTypes().size());
    assertEquals("There should be 22 criteria", 22, covGath.getCoverageCriteria().size());
    assertEquals(
        "There should be 63 elements to cover",
        63,
        covGath.getCoverageCriteria().stream()
            .mapToInt(criterion -> criterion.getElements().size())
            .sum());
  ...
  public void coverageGathererComplexComplexRefResponseBodyPropertiesTest() {
    ...
    assertTrue(
        covGath.getCoverageCriteria().stream()
                .filter(
                    cc ->
                        cc.getType() == RESPONSE_BODY_PROPERTIES
                            && cc.getRootPath()
                                .equals("/find-by-keyword-id->GET->200->{place{address{"))
                .findFirst()
                .get()
                .getElements()
                .size()
        ...
        covGath.getCoverageCriteria().stream()
            .filter(
                cc ->
                    cc.getType() == RESPONSE_BODY_PROPERTIES
                        && cc.getRootPath()
                            .equals("/find-by-keyword-id->GET->200->{place{address{countryCode"))
            .findFirst()
            .isPresent());

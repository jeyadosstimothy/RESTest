...
public class CoverageMeterTest {
  ...
  public void setUp() {
    ...
    testCase2 =
        new TestCase(generateId(), false, "findPetsByStatus", "/pet/findByStatus", HttpMethod.GET);
    testCase2.addQueryParameter("status", "available");
    testCase3 =
        new TestCase(
            generateId(), false, "uploadFile", "/pet/{petId}/uploadImage", HttpMethod.POST);
    ...
    testResult3 =
        new TestResult(
            testCase3.getId(), "200", "{\"a\": \"b\", \"c\": \"d\"}", "application/json");
  ...
  public void coverageMeterTest() {
    ...
    System.out.println(
        "covMeter.getCriterionTypeCoverage(PATH): " + covMeter.getCriterionTypeCoverage(PATH));
    System.out.println(
        "covMeter.getCriterionTypeCoverage(OPERATION): "
            + covMeter.getCriterionTypeCoverage(OPERATION));
    System.out.println(
        "covMeter.getCriterionTypeCoverage(PARAMETER): "
            + covMeter.getCriterionTypeCoverage(PARAMETER));
    System.out.println(
        "covMeter.getCriterionTypeCoverage(PARAMETER_VALUE): "
            + covMeter.getCriterionTypeCoverage(PARAMETER_VALUE));
    System.out.println(
        "covMeter.getCriterionTypeCoverage(INPUT_CONTENT_TYPE): "
            + covMeter.getCriterionTypeCoverage(INPUT_CONTENT_TYPE));
    System.out.println(
        "covMeter.getCriterionTypeCoverage(STATUS_CODE_CLASS): "
            + covMeter.getCriterionTypeCoverage(STATUS_CODE_CLASS));
    System.out.println(
        "covMeter.getCriterionTypeCoverage(STATUS_CODE): "
            + covMeter.getCriterionTypeCoverage(STATUS_CODE));
    System.out.println(
        "covMeter.getCriterionTypeCoverage(OUTPUT_CONTENT_TYPE): "
            + covMeter.getCriterionTypeCoverage(OUTPUT_CONTENT_TYPE));
    System.out.println(
        "covMeter.getCriterionTypeCoverage(RESPONSE_BODY_PROPERTIES): "
            + covMeter.getCriterionTypeCoverage(RESPONSE_BODY_PROPERTIES));
    System.out.println(
        "covMeter.getCriterionCoverage(OPERATION, /pet): "
            + covMeter.getCriterionCoverage(OPERATION, "/pet"));
    System.out.println(
        "covMeter.getCriterionCoverage(PARAMETER, /pet/findByStatus->GET): "
            + covMeter.getCriterionCoverage(PARAMETER, "/pet/findByStatus->GET"));
    System.out.println(
        "covMeter.getCriterionCoverage(PARAMETER, /pet/{petId}/uploadImage->POST): "
            + covMeter.getCriterionCoverage(PARAMETER, "/pet/{petId}/uploadImage->POST"));
    System.out.println(
        "covMeter.getCriterionCoverage(PARAMETER_VALUE,"
            + " /pet/{petId}/uploadImage->POST->destinationFormat): "
            + covMeter.getCriterionCoverage(
                PARAMETER_VALUE, "/pet/{petId}/uploadImage->POST->destinationFormat"));
    System.out.println(
        "covMeter.getCriterionCoverage(STATUS_CODE, /pet/findByStatus->GET): "
            + covMeter.getCriterionCoverage(STATUS_CODE, "/pet/findByStatus->GET"));
    System.out.println(
        "covMeter.getCriterionCoverage(RESPONSE_BODY_PROPERTIES, /pet->POST->201): "
            + covMeter.getCriterionCoverage(RESPONSE_BODY_PROPERTIES, "/pet->POST->201"));
    System.out.println(
        "covMeter.getCriterionCoverage(RESPONSE_BODY_PROPERTIES, /pet/findByStatus->GET->200): "
            + covMeter.getCriterionCoverage(
                RESPONSE_BODY_PROPERTIES, "/pet/findByStatus->GET->200"));
    System.out.println(
        "covMeter.getCriterionCoverage(RESPONSE_BODY_PROPERTIES,"
            + " /pet/{petId}/uploadImage->POST->200): "
            + covMeter.getCriterionCoverage(
                RESPONSE_BODY_PROPERTIES, "/pet/{petId}/uploadImage->POST->200"));
  }
  ...
  public void exportCoverageTest() {
    ...
    CoverageMeter.exportCoverageOfTestCaseToCSV(
        "src/test/resources/csvData/test-cases-coverage.csv", testCase2);
    ...
    CoverageMeter.exportCoverageOfTestResultToCSV(
        "src/test/resources/csvData/test-results-coverage.csv", testResult3);
  ...
  public void coverageResultsTest() {
    ...
    CoverageResults coverageResults =
        new CoverageResults(
            covMeter.getTotalCoverage(), covMeter.getInputCoverage(), covMeter.getOutputCoverage());
  ...
  public void addTestSuiteTest() {
    ...
    TestCase tc4 =
        new TestCase(generateId(), false, "getOrderById", "/store/order/{orderId}", HttpMethod.GET);
    newTestSuite.add(tc4);

    covMeter.addTestSuite(newTestSuite);

    assertTrue(
        "The new coverage should be higher than the old one",
        covMeter.getTotalCoverage() > oldCoverage);
  }
  ...
  public void resetTestSuiteTest() {
    ...
    assertTrue(
        "The new coverage should be lower than the old one",
        covMeter.getTotalCoverage() < oldCoverage);
  }
}

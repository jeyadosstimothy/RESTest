...
public class AllureReportManager {
  ...
  private Boolean historyTrend =
      false; // If true, it show history information by copying the 'history' directory from
  // previous report
  private Boolean loadCategories =
      true; // If true, it uses the custom categories defined in the properties file located in
  // src/main/resources
  ...
  public AllureReportManager() {
    this(
        PropertyManager.readProperty("allure.results.dir"),
        PropertyManager.readProperty("allure.report.dir"),
        new ArrayList<>());
  }
  ...
  public AllureReportManager(String resultsDir, String reportDir, List<String> authProperties) {
    ...
    else allureCommand = PropertyManager.readProperty("allure.command.unix");
  }

  public void generateReport() {

    // If history trend is enabled, we must copy the "history" directory from the current report to
    // the allure results directory
    if (historyTrend) copyHistoryDirectory();

    // If category loading is enabled, we must copy the file "allure-categories.json" to the allure
    // results directory.
    if (loadCategories) copyCategoryFile();

    // Generate report
    Runtime rt = Runtime.getRuntime();
    try {
      Process proc =
          rt.exec(allureCommand + " generate -c " + resultsDirPath + " -o " + reportDirPath);
      proc.waitFor();
  ...
  // Copy the files allure-categories.json file from the resource directory to the allure results
  // directory
  ...
  // Copy the history subfolder of the allure report to the allure results directory to enable the
  // trend view.

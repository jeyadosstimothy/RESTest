...
/**
 * This class generates the statistics related to the generation and execution of test cases.
 * Currently, it generates test cases in CSV format and coverage data
 */
public class StatsReportManager {
  ...
  public void generateReport(String testId, boolean executeTestCases) {
    ...
    // Generate CVS stats
    if (enableCSVStats) generateCSVStats(testId);
  ...
  private void generateCSVStats(String testId) {
    ...
    String csvTcPath =
        testDataDir
            + "/"
            + PropertyManager.readProperty("data.tests.testcases.file")
            + "_"
            + testId
            + ".csv";
  ...
  private void generateCoverageStats(String testId, boolean executeTestCases) {
    ...
    if (enableInputCoverage || enableOutputCoverage) {
      ...
      if (executeTestCases) {
        ...
        exportCoverageReport(
            coverageMeter.getAPosteriorCoverageMeter(),
            coverageDataDir
                + "/"

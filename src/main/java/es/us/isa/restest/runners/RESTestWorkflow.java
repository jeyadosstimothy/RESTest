...
/**
 * This class implements a basic test workflow: test generation -> test writing -> class compilation
 * and loading -> test execution -> test report generation -> test coverage report generation.
 *
...
public class RESTestWorkflow {
  ...
  private boolean allureReports =
      true; // Whether to actually generate reports or not (folder "allure-reports")
  ...
  public RESTestWorkflow(
      ...
      IWriter writer,
      AllureReportManager reportManager,
  ...
  protected void testExecution(Class<?> testClass) {
    ...
    logger.info(
        "{} tests run in {} seconds. Successful: {}, Failures: {}, Ignored: {}",
        result.getRunCount(),

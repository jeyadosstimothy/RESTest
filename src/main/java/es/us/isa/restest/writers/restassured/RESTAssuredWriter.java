...
/**
 * This class defines a test writer for the REST Assured framework. It creates a Java class with
 * JUnit test cases ready to be executed.
 *
...
public class RESTAssuredWriter implements IWriter {
  ...
  private boolean allureReport =
      false; // Generate request and response attachment for allure reports
  ...
  private boolean
      logToFile; // If 'true', REST-Assured requests and responses will be logged into external
  // files
  ...
  public RESTAssuredWriter(
      String specPath,
      String testConfPath,
      String targetDirJava,
      String className,
      String packageName,
      String baseURI,
      Boolean logToFile) {
  ...
  public void write(Collection<TestCase> testCases) {
    ...
    // Generate tests
    int ntest = 1;
    for (TestCase t : testCases) contentFile += generateTest(t, ntest++);
  ...
  private String generateImports(String packageName) {
    ...
    if (packageName != null) content += "package " + packageName + ";\n\n";
    ...
    content +=
        "import org.junit.*;\n"
            ...
            + "import"
            + " es.us.isa.restest.writers.restassured.filters.NominalOrFaultyTestCaseFilter;\n"
  ...
  private String generateAttributes(String specPath) {
    ...
    //		if (OAIValidation)
    content +=
        "\tprivate static final String OAI_JSON_URL = \""
            + specPath
            + "\";\n"
            + "\tprivate static final StatusCode5XXFilter statusCode5XXFilter = new"
            + " StatusCode5XXFilter();\n"
            + "\tprivate static final NominalOrFaultyTestCaseFilter nominalOrFaultyTestCaseFilter ="
            + " new NominalOrFaultyTestCaseFilter();\n"
            + "\tprivate static final ResponseValidationFilter validationFilter = new"
            + " ResponseValidationFilter(OAI_JSON_URL);\n";
    if (statefulFilter)
      content +=
          "\tprivate static final StatefulFilter statefulFilter = new StatefulFilter(\""
    ...
    if (enableStats
        || enableOutputCoverage) { // This is only needed to export output data to the proper folder
      content +=
  ...
  private String generateFiltersInitialization(TestCase t) {
    ...
    content +=
        "\t\tnominalOrFaultyTestCaseFilter.updateFaultyData("
            + t.getFaulty()
            + ", "
            + t.getFulfillsDependencies()
            + ", \""
            + escapeJava(t.getFaultyReason())
            + "\");\n"
            + "\t\tstatusCode5XXFilter.updateFaultyData("
            + t.getFaulty()
            + ", "
            + t.getFulfillsDependencies()
            + ", \""
  ...
  private String generateRESTAssuredObject(TestCase t) {
    ...
    if (logging && !logToFile) content += "\t\t\t\t.log().all()\n";
  ...
  private String generateHeaderParameters(TestCase t) {
    ...
    for (Entry<String, String> param : t.getHeaderParameters().entrySet())
      content +=
          "\t\t\t\t.header(\"" + param.getKey() + "\", \"" + escapeJava(param.getValue()) + "\")\n";
  ...
  private String generateBodyParameter(TestCase t) {
    ...
    if ((t.getFormParameters() == null || t.getFormParameters().size() == 0)
        ...
        (t.getMethod().equals(HttpMethod.POST)
            || t.getMethod().equals(HttpMethod.PUT)

...
/**
 * REST-Assured filter for validating ONLY responses from the API. The original Atlassian
 * OpenApiValidationFilter validates both the request and response. We only want to validate the
 * response, since we may intentionally send invalid inputs to the API in order to create faulty
 * test cases. Request body validation errors (e.g. omitting a required property) will be
 * whitelisted, while other errors will be emitted as warnings.
 *
...
public class ResponseValidationFilter extends RESTestFilter implements OrderedFilter {
  ...
  private final Boolean enabled =
      Boolean.parseBoolean(PropertyManager.readProperty("response.body.analysis"));
  private final Boolean limit =
      Boolean.parseBoolean(PropertyManager.readProperty("response.body.limit"));
  private final Long bodySizeLimit =
      Long.parseLong(PropertyManager.readProperty("response.body.size"));
  ...
  public Response filter(
    ...
    if (enabled && (!limit || response.getBody().asString().length() < bodySizeLimit))
      filterValidation(
          response, restAssuredRequest.getPath(), restAssuredRequest.getMethod().toString());
  ...
  public void filterValidation(Response response, String path, String method) {
    final ValidationReport validationReport =
        validator.validateResponse(
            path, Request.Method.valueOf(method), RestAssuredResponse.of(response));
    if (validationReport.hasErrors()) {
      String errors = "OAS disconformity: " + getMessagesSummary(validationReport);
      if (APIName != null && testResultId != null) exportTestResultToCSV(response, false, errors);
  ...
  public int getOrder() {
    return Integer.MAX_VALUE
        - 3; // Fourth lowest priority of all filters, so it runs fourth-to-last before sending the
    // request and fourth after sending it
  }
}

...
/**
 * This class defines a test writer for Postman. It creates a JSON file with a collection of Postman
 * requests.
 *
...
public class PostmanWriter implements IWriter {
  ...
  private static final String postmanSchema =
  ...
  private PostmanCollectionObject createPostmanTestSuite(Collection<TestCase> testCases) {
    ...
    for (TestCase tc : testCases) {
      ...
      // Request body
      if ((tc.getBodyParameter() != null && !tc.getBodyParameter().equals(""))
          || !tc.getFormParameters().isEmpty()) request.setBody(getPostmanBody(tc));
  ...
  private Url getPostmanUrl(TestCase tc) {
    ...
    for (Map.Entry<String, String> pathParam : tc.getPathParameters().entrySet())
      pathParamsString =
          pathParamsString.replace("{" + pathParam.getKey() + "}", pathParam.getValue());
    ...
    for (Map.Entry<String, String> queryParam : tc.getQueryParameters().entrySet()) {
      ...
      try {
        ...
        queryParamValue =
            URLEncoder.encode(queryParam.getValue(), StandardCharsets.UTF_8.toString());
      } catch (UnsupportedEncodingException e) {
        logger.error(
            "Query parameter {} could not be URLencoded. Adding it without encoding.",
            queryParam.getKey());
  ...
  private Body getPostmanBody(TestCase tc) {
    ...
    if (tc.getInputFormat().equals("application/json")) {
      ...
      for (Map.Entry<String, String> tcFormParameter : tc.getFormParameters().entrySet()) {
        formParameters.add(
            new Urlencoded(tcFormParameter.getKey(), tcFormParameter.getValue(), "text"));
      }
  ...
  private void saveToFile(String path, String fileName, PostmanCollectionObject content) {
    ...
    try {
      objectMapper.writeValue(
          new File(path + "/" + fileName + ".postman_collection.json"), content);
    } catch (IOException e) {

...
public class StatefulFilter extends RESTestFilter implements OrderedFilter {
        ...
        if (jsonFile.exists())
          allValues =
              objectMapper.readValue(
                  jsonFile, new TypeReference<Map<String, Map<String, List<JsonNode>>>>() {});
  ...
  private void addResponseBodyValues(
      Map<String, List<JsonNode>> allValues, JsonNode bodyNode, String prefix) {
    if (bodyNode.isObject()) {
      ...
      if (!allValues.get(prefix).contains(bodyNode)) allValues.get(prefix).add(bodyNode);
    }
  }
  ...
  public int getOrder() {
    return Integer
        .MAX_VALUE; // Lowest priority of all filters, so it runs last before sending the request
    // and first after sending it
  }
}

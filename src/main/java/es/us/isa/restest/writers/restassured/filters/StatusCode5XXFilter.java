...
public class StatusCode5XXFilter extends RESTestFilter implements OrderedFilter {
    ...
    if (response.getStatusCode() >= 500) {
      if (testCaseIsFaulty != null && testCaseIsFaulty)
        saveTestResultAndThrowException(
            response, "Status code 5XX with invalid request: " + faultyReason);
  ...
  public int getOrder() {
    return Integer.MAX_VALUE
        - 1; // Second lowest priority of all filters, so it runs second-to-last before sending the
    // request and second after sending it
  }
}

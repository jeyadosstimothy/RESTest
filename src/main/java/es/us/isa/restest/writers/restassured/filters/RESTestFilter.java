...
/**
 * REST-Assured filter to be extended by all RESTest filters. This class implements a method to
 * export a REST-Assured response to a TestResult in CSV. Such method must be called before throwing
 * the exception corresponding to the oracle being violated.
 */
public class RESTestFilter {
  ...
  protected Boolean
      dependenciesFulfilled; // Whether this test case fulfills all inter-parameter dependencies or
  // not
  ...
  protected void exportTestResultToCSV(Response response, Boolean passed, String failReason) {
    String testDataFile =
        PropertyManager.readProperty("data.tests.dir")
            ...
            + APIName
            + "/"
            + PropertyManager.readProperty("data.tests.testresults.file")
            + "_"
            + testId
            + ".csv";
    TestResult tr =
  ...
  protected void saveTestResultAndThrowException(Response response, String message) {
    if (APIName != null && testResultId != null) exportTestResultToCSV(response, false, message);
    throw new RuntimeException(message);
  }

  public void updateFaultyData(
      Boolean testCaseIsFaulty, Boolean dependenciesFulfilled, String faultyReason) {
    this.testCaseIsFaulty = testCaseIsFaulty;
    this.dependenciesFulfilled = dependenciesFulfilled;
    this.faultyReason = faultyReason;

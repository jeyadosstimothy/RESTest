...
public class TestParameter {
  ...
  public void addRegexToTestParameter(String regex) {
    ...
    for (Generator generator : this.generators) {
      if (Boolean.TRUE.equals(generator.isValid())
          && generator.getType().equals(RANDOM_INPUT_VALUE)) {
        for (GenParameter genParameter : generator.getGenParameters()) {
          if (genParameter.getName().equals(PREDICATES)) {

            GenParameter oldGenParameter =
                generator.getGenParameters().stream()
                    .filter(x -> x.getName().equals("regExp"))
                    .findFirst()
                    .orElse(null);

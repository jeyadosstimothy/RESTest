...
/**
 * TestConfiguration objects are key in RESTest. They include all the information required to test
 * an API (data dictionaries, authentication data, etc), complementing the information provided by
 * the API specification. This class manage the generation of a default test configuration file from
 * an OpenAPI specification. Default test configuration files are currently pretty simply and should
 * be manually updated before testing (ex. adding specific data dictionaries).
 */
public class DefaultTestConfigurationGenerator {
  ...
  private static final Logger logger =
      LogManager.getLogger(DefaultTestConfigurationGenerator.class);
  ...
  public TestConfigurationObject generate(
      String destination, Collection<TestConfigurationFilter> filters) {
  ...
  // Generate the test configuration data for the target paths and operations (as indicated in the
  // filters)
    ...
    for (TestConfigurationFilter filter : filters) {
      ...
      for (Entry<String, PathItem> path : paths.entrySet())
        if (filter.getPath() == null || path.getKey().equalsIgnoreCase(filter.getPath()))
          operations.addAll(
              generateOperationsOfPath(path, filter.getMethods())); // For every filter, add its
      // path to testConf
    }
    return operations;
  }
  ...
  private List<Operation> generateOperationsOfPath(
      Entry<String, PathItem> path, Collection<HttpMethod> methods) {

    List<Operation> pathOperations = new ArrayList<>();

    for (Entry<HttpMethod, io.swagger.v3.oas.models.Operation> operationEntry :
  ...
  private Operation generateOperation(
      Entry<HttpMethod, io.swagger.v3.oas.models.Operation> operationEntry, String path) {
    ...
    else testOperation.setOperationId("<SET OPERATION ID>");
    ...
    if (operationEntry.getValue().getParameters() != null) {
      testOperation.setTestParameters(
          generateTestParameters(operationEntry.getValue().getParameters()));
    }

    // Set request body parameters
    if (operationEntry.getValue().getRequestBody() != null
        && testOperation.getTestParameters() == null) {
      testOperation.setTestParameters(
          generateRequestBodyTestParameters(operationEntry.getValue().getRequestBody()));
  ...
  private List<TestParameter> generateTestParameters(List<Parameter> parameters) {
    ...
    for (Parameter param : parameters) {
      ...
      if ((param.getIn().equals("query")
        ...
        if (param.getRequired() == null || !param.getRequired()) testParam.setWeight(0.5f);
  ...
  private List<TestParameter> generateObjectParameters(
      Schema schema,
      String objectName,
      String in,
      Boolean required,
      Parameter.StyleEnum styleParam,
      Encoding.StyleEnum styleFormData,
      Boolean explode) {
    ...
    if (schema.getProperties() != null) {
      for (Object o : schema.getProperties().entrySet()) {
        ...
        if (required == null
            ...
            || !schema.getRequired().contains(property.getKey())) propertyParam.setWeight(0.5f);
  ...
  private void generateStringGenerator(
      Generator gen, String format, String pattern, Integer minLength, Integer maxLength) {
    GenParameter genParam1 = new GenParameter();
    if (pattern != null) {
  ...
  private void generateNumberGenerator(
      Generator gen,
      String format,
      BigDecimal minimum,
      BigDecimal maximum,
  ...
  private List<TestParameter> generateRequestBodyTestParameters(RequestBody requestBody) {
    ...
    if (requestBody.getContent().keySet().stream()
      ...
      if (requestBody.getContent().keySet().stream()
          .anyMatch(x -> x.matches(MEDIA_TYPE_APPLICATION_JSON_REGEX))) {
        Entry<String, MediaType> mediaTypeEntry =
            requestBody.getContent().entrySet().stream()
                .filter(x -> x.getKey().matches(MEDIA_TYPE_APPLICATION_JSON_REGEX))
                .findFirst()
                .orElse(null);
        if (mediaTypeEntry != null) addObjectPerturbatorIfPossible(gens, mediaTypeEntry.getValue());
        ...
        Map.Entry<String, MediaType> mediaTypeEntry =
            requestBody.getContent().entrySet().stream()
                .filter(x -> x.getKey().matches(MEDIA_TYPE_TEXT_PLAIN_REGEX))
                .findFirst()
                .orElse(null);
        if (mediaTypeEntry != null) generateGenerator(gen, mediaTypeEntry.getValue().getSchema());
      ...
      for (Object entry : mediaType.getSchema().getProperties().entrySet()) {
        ...
        if ("object".equals(parameterSchema.getType())) {
          Encoding encoding =
              mediaType.getEncoding() != null ? mediaType.getEncoding().get(parameterName) : null;
      ...
      try {
        bodyParam =
            objectMapper.writeValueAsString(
                mediaType.getExamples().entrySet().iterator().next().getValue());
        ...
        bodyParam =
            objectMapper.writeValueAsString(
                spec.getSpecification()
                    .getComponents()
                    .getSchemas()
                    .get(bodyReference.replace("#/components/schemas/", ""))
                    .getExample());
      } catch (JsonProcessingException e) {
        logger.error(e.getMessage(), e);
      }

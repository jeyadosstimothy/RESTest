...
public class TestConfigurationVisitor {
  ...
  /**
   * Returns the test configuration object for a given operation (from the list of operations of the
   * path)
   *
  ...
  public static List<Operation> getOperationsOfTestPath(TestConfigurationObject conf, String path) {
    List<Operation> ops =
        conf.getTestConfiguration().getOperations().stream()
            .filter(x -> x.getTestPath().equalsIgnoreCase(path))
            .collect(Collectors.toList());

    if (ops.isEmpty()) {
      throw new IllegalArgumentException(
          "Path <" + path + "> does not exist in test configuration file");
    }
  ...
  /**
   * Returns the test configuration parameter with name=paramName of the operation with
   * id=operationId
   *
   * @param conf General test configuration object
   ...
   */
  public static TestParameter getTestParameter(
      TestConfigurationObject conf, String operationId, String paramName) {
  ...
  /**
   * Search a generator's configuration parameter in a list of parameters (or null if it does not
   * exist)
   *
   ...
   */
  public static GenParameter searchGenParameter(
      String paramName, List<GenParameter> genParameters) {
   ...
   * @return Test configuration parameter
   */
  public static TestParameter searchTestParameter(
      String paramName, List<TestParameter> testParameters) {
  ...
  /**
   * Analyzes the whole testConf to look for either ParameterGenerator or BodyGenerator. If there's
  ...
  public static boolean hasStatefulGenerators(TestConfigurationObject conf) {
    return conf.getTestConfiguration().getOperations().stream()
  ...
  public static boolean hasStatefulGenerators(Operation operation) {
    try {
      return operation.getTestParameters().stream()
          .anyMatch(
              p ->
                  p.getGenerators().stream()
                      .anyMatch(
                          g ->
                              g.getType().equals("BodyGenerator")
                                  || g.getType().equals("ParameterGenerator")));
  ...
  /**
   * Analyzes the whole testConf to look for RandomInputValue generators using ARTE parameters
   * (i.e., predicates or numberOfTriesToGenerateRegex). If there's any of those, returns true,
   * otherwise returns false.
   */
  public static boolean isArteEnabled(TestConfigurationObject conf) {
    return conf.getTestConfiguration().getOperations().stream()
        .anyMatch(TestConfigurationVisitor::isArteEnabled);
  }
    ...
    try {
      return operation.getTestParameters().stream()
          .anyMatch(
              p ->
                  p.getGenerators().stream()
                      .filter(g -> g.getType().equals("RandomInputValue"))
                      .anyMatch(
                          g ->
                              g.getGenParameters().stream()
                                  .anyMatch(
                                      gp ->
                                          gp.getName().equals("predicates")
                                              || gp.getName()
                                                  .equals("numberOfTriesToGenerateRegex"))));

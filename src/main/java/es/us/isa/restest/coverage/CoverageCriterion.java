...
/**
 * Class that represents a specific coverage criterion, e.g. paths, parameters, etc. Keeps track of
 * the coverage level
 *
...
public class CoverageCriterion {
  ...
  private Map<String, Boolean>
      elements; // Keys are the elements to cover and values represent whether they have already

  // been covered or not

  /**
   * The following property's purpose is to locate the criterion inside the API resources hierarchy.
   * There could be several parameter values criteria having all the same parameter name. Therefore,
   * it is necessary to uniquely identify each criterion to check if they are covered when analysing
   * the abstract test cases. This property makes sense for all criteria except for paths, since
   * there is only one of this kind. The longest allowed rootPath contains 3 elements:
   * "{path}->{operationId}->{parameterName}" (parameter value criterion) OR
  ...
  public long getCoveredElementsCount() {
    return elements.entrySet().stream().filter(e -> e.getValue()).count();
  }
  ...
  /**
   * Get coverage of this criterion as a percentage by dividing the number of covered elements by

 ...
 * coverage, the input coverage, the output coverage and the coverage of every coverage criterion,
 * plus the coverage of every criterion type and the Test Coverage Level (TCL).
 */
public class CoverageResults {

  private int
      coverageLevel; // The coverage level of the test. It is defined by the test coverage model
  ...
  private float
      inputContentTypeCoverage; // The input content type (application/json...) coverage of the test
  private float
      outputContentTypeCoverage; // The output content type (application/json...) coverage of the
  // test
  private float
      parameterCoverage; // The parameter (input parameters of an operation, e.g. lat, lon,
  // order...) coverage of the test
  private float statusCodeClassCoverage; // The status code class (2XX,...) coverage of the test
  private float
      parameterValueCoverage; // The parameter value (values of an enum, booleans...) coverage of
  // the test
  ...
  private float
      responseBodyPropertiesCoverage; // The response body properties (properties of the JSON
  // response) coverage of the test
  ...
  public void setCoverageOfCoverageCriteria(
      List<CoverageCriterionResult> coverageOfCoverageCriteria) {
  ...
  /**
   * This method sets the map that defines the coverage of every coverage criteria from a
   * CoverageMeter.
   *
  ...
  public void setCoverageOfCoverageCriteriaFromCoverageMeter(CoverageMeter covMeter) {
    ...
    for (CoverageCriterion c : criteria) {
      ...
      CoverageCriterionResult res =
          new CoverageCriterionResult(
              typeAndRootPath, covMeter.getCriterionCoverage(c.getType(), c.getRootPath()));
      coverageOfCoverageCriteria.add(res);
    }
  }

  /**
   * This method sets the coverage of every criterion type from a CoverageMeter. Once the method has
   * finished, it calls another method to set the test coverage level.
   *
   * @param covMeter the CoverageMeter with the coverage computation
   */
  ...
  /**
   * This method sets the test coverage level. The coverage level is defined by the Test Coverage
   * Model (TCM): - Level 0 represents a test suite where no coverage criterion is fully met.
   * (Default level) - Level 1 requires paths to be fully covered. - Level 2 requires level 1 and
   * every operation to be covered. - Level 3 requires level 2 and a 100% coverage of both input and
   * output content-type criteria. - Level 4 requires level 3 and a 100% coverage of both parameters
   * and status code classes criteria. - Level 5 requires level 4 and every parameter value and
   * status code to be covered. - Level 6 requires level 5 and a 100% coverage of response body
   * properties criteria. - Level 7: in a distant future.
   */
  ...
  public void exportCoverageReportToCSV(String path) {
    if (!checkIfExists(path)) { // If the file doesn't exist, create it (only once)
      StringBuilder header =
          new StringBuilder(
              "coverageLevel,totalCoverage,inputCoverage,outputCoverage,pathCoverage,operationCoverage,inputContentTypeCoverage,"
    ...
    StringBuilder row =
        new StringBuilder(
            coverageLevel
                ...
                + operationCoverage
                + ","
                + inputContentTypeCoverage
                + ","
                + outputContentTypeCoverage
                + ","
                + parameterCoverage
                + ","
                + statusCodeClassCoverage
                + ","

...
public class CoverageMeter {
  ...
  private CoverageGatherer
      coverageGatherer; // coverage gatherer already containing all criteria to be covered
  private Collection<TestCase> testSuite; // full set of abstract test cases addressing the API
  private Collection<TestResult>
      testResults; // test outputs generated after running the test suite against the API
  ...
  public CoverageMeter(CoverageGatherer coverageGatherer, Collection<TestCase> testSuite) {
    ...
    setCoveredInputElements(
        testSuite); // after setting testSuite, update covered input elements from all criteria
  }

  public CoverageMeter(
      CoverageGatherer coverageGatherer,
    ...
    setCoveredOutputElements(
        testResults,
        testSuite); // after setting testResults, update covered output elements from all criteria
  }
  ...
  public float getCriterionCoverage(CriterionType type, String rootPath) {
    CoverageCriterion criterion =
        coverageGatherer.getCoverageCriteria().stream() // find criterion
  ...
  /**
   * Based on {@code this} CoverageMeter object, returns a modified CoverageMeter whose input
   * coverage counts only those elements whose response was successful.
   *
  ...
  public CoverageMeter getAPosteriorCoverageMeter(int maxTestSuiteSize) {
    CoverageMeter aPosterioriCoverageMeter =
        new CoverageMeter(new CoverageGatherer(coverageGatherer.getSpec()));
    ...
    if (testResults != null) {
      ...
      for (int i = 0; i * maxTestSuiteSize < totalSize; i++) {
        ...
        Collection<TestCase> testSuiteFragment =
            orderedTestSuite.subList(i * maxTestSuiteSize, upperLimit);
        Collection<TestResult> testResultsFragment =
            orderedTestResults.subList(i * maxTestSuiteSize, upperLimit);

        List<String> invalidResponseResultsIds =
            new ArrayList<>(testResultsFragment)
                .stream()
  ...
  private void setCoveredInputElements(Collection<TestCase> testSuite) {
    // Traverse all test cases and, for each one, modify the coverage criteria it affects, by adding
    // new covered elements
    for (TestCase testCase : testSuite) {
      updateCriterion(PATH, "", testCase.getPath(), coverageGatherer);
      updateCriterion(
          OPERATION, testCase.getPath(), testCase.getMethod().toString(), coverageGatherer);
      for (Entry<String, String> parameter : testCase.getHeaderParameters().entrySet()) {
        updateCriterion(
            PARAMETER,
            testCase.getPath() + "->" + testCase.getMethod().toString(),
            parameter.getKey(),
            coverageGatherer);
        updateCriterion(
            PARAMETER_VALUE,
            testCase.getPath() + "->" + testCase.getMethod().toString() + "->" + parameter.getKey(),
            parameter.getValue(),
            coverageGatherer);
      }
      ...
      updateCriterion(
          ...
          "body",
          coverageGatherer);
      //            updateCriterion(AUTHENTICATION, testCase.getPath() + "->" +
      // testCase.getMethod().toString(), testCase.getAuthentication());
      updateCriterion(
          INPUT_CONTENT_TYPE,
          testCase.getPath() + "->" + testCase.getMethod().toString(),
          testCase.getInputFormat(),
          coverageGatherer);
    }
  }

  private void setCoveredOutputElements(
      Collection<TestResult> testResults, Collection<TestCase> testSuite) {
    // Traverse all test results and, for each one, modify the coverage criteria it affects, by
    // adding new covered elements
    for (TestResult testResult : testResults) {
      ...
      if (statusCodeClass != null)
        updateCriterion(
            STATUS_CODE_CLASS,
            findTestCase(testResult.getId(), testSuite).getPath()
                + "->"
      ...
      updateCriterion(
          ...
          findTestCase(testResult.getId(), testSuite).getPath()
              + "->"
              + findTestCase(testResult.getId(), testSuite).getMethod().toString(),
          outputContentTypeTranslator(testResult.getOutputFormat()),
          coverageGatherer);
      ...
      try {
        ...
        //                System.out.println("Error setting response body properties coverage
        // criteria: response body is text/html; expected application/json.");
        //                e.printStackTrace();
      }
    }
  }
  ...
  /**
   * This method checks if the outputFormat is application/json or application/xml; in any of those
   * cases, returns the output format that CoverageMeter is able to manage.
   */
   ...
   * in the CoverageGatherer of the class, or b) to export that same data to CSV. Given a JSON node,
   * if it is a JSON object or an array of JSON objects, apply one of the two processes mentioned
   * for every RESPONSE_BODY_PROPERTIES criterion affected. This function is to be called
   * recursively, so as to cover all sub-properties of the root element.
   *
   * @param jsonNode JsonNode object that may contain some properties which will be marked as
   *     'covered' on the {@link CoverageGatherer} object, or exported to CSV. A JsonNode can be a
   *     JSON object, an array or any other data type (such as an integer or a string)
   * @param baseRootPath Case a): Initial rootPath: "{path}->{httpMethod}->{statusCode}->". Example
   *     of baseRootPath after 2 iterations: "{path}->{httpMethod}->{statusCode}->{prop1[{prop2".
   *     Case b): Initial rootPath: "". Example of baseRootPath after 2 iterations: "{prop1[{prop2"
   ...
   * @param testResultId ID of the test result that is being exported (must be added in the row of
   *     the CSV file). {@code null} for case b)
   * @param coveredRootPaths List of rootPaths that have already been covered (added to the CSV
   *     file). This is necessary because, for an array, the same element could be written multiple
   *     times (e.g. thousands of duplicated lines for an array with thousands of elements). {@code
   *     null} for case b)
   */
  private static void iterateOverJsonNode(
      JsonNode jsonNode,
      String baseRootPath,
      CoverageGatherer covGath,
      String filePath,
      String testResultId,
      List<String> coveredRootPaths) {
    ...
    if (jsonNode instanceof ObjectNode) { // if the jsonNode is actually a JSON object
      ...
      updateResponseBodyPropertiesCriteria(
        ...
        && jsonNode.get(0) instanceof ObjectNode) { // if the jsonNode is an array of JSON objects
  ...
  /**
   * Given some object properties, iterate over all of them and update the affected
   * RESPONSE_BODY_PROPERTIES criteria accordingly. For each property, iterate over all of their
   * sub-properties again (recursively) to continue covering all sub-properties.
   *
   * @param objectPropertiesIterator Iterator of entries where the key is the name of the property
   *     and the value is the JsonNode property
   * @param rootPath Root path to locate the coverage criterion among all criteria of the {@link
   *     CoverageGatherer} object. To be updated with the name of the property when calling {@link
   *     #iterateOverJsonNode(JsonNode, String, CoverageGatherer, String, String, List)
   *     iterateOverJsonNode}
   * @param covGath CoverageGatherer where to update the criteria
   * @param filePath Path to the CSV file where to export the data
   * @param testResultId ID of the test result that is being exported (must added in the row of the
   *     CSV file).
   * @param coveredRootPaths List of rootPaths that have already been covered (added to the CSV
   *     file). This is necessary because, for an array, the same element could be written multiple
   *     times (e.g. thousands of duplicated lines for an array with thousands of elements).
   */
  private static void updateResponseBodyPropertiesCriteria(
      Iterator<Entry<String, JsonNode>> objectPropertiesIterator,
      String rootPath,
      CoverageGatherer covGath,
      String filePath,
      String testResultId,
      List<String> coveredRootPaths) {
    while (objectPropertiesIterator != null
        && objectPropertiesIterator.hasNext()) { // iterate over all properties of the object
      ...
      updateCriterion(
            ...
            rootPath,
            responseProperty.getKey(),
            covGath); // set the property as 'covered'
        ...
        if (!coveredRootPaths.contains(updatedRootPath)) {
          writeCSVRow(
              filePath,
              testResultId + ",RESPONSE_BODY_PROPERTIES," + rootPath + responseProperty.getKey());
          coveredRootPaths.add(
              updatedRootPath); // add the rootPath to the list of covered ones, in order not to
          // duplicate lines in the CSV file
        }
      }
      iterateOverJsonNode(
          responseProperty.getValue(),
          rootPath + responseProperty.getKey(),
          covGath,
          filePath,
          testResultId,
          coveredRootPaths); // iterate over the sub-properties of that property
    }
  }
   ...
   */
  private static void updateCriterion(
      CriterionType type, String rootPath, String element, CoverageGatherer covGath) {
    // Find unique criterion by type and rootPath
    CoverageCriterion criterion =
        covGath.getCoverageCriteria().stream()
            .filter(c -> c.getType() == type && c.getRootPath().equals(rootPath))
            .findFirst()
            .orElse(null);

    if (criterion != null) { // if the criterion exists
      criterion.coverElement(
          element); // add element to the already covered elements of the criterion
    }
  }
  ...
  private TestCase findTestCase(String id, Collection<TestCase> testSuite) {
    return testSuite.stream()
        ...
        .findFirst()
        .orElseThrow(
            () -> new IllegalArgumentException("There is no test case with id <" + id + ">"));
  }

  /**
   * Export coverage data to external file (CSV). You can export input, output or both types of
   * coverage data.
   *
  ...
  public void exportCoverageToCSV(String path, String criterionType, boolean overwrite) {
    if (overwrite) createCSVwithHeader(path, "criterionType,rootPath,element,isCovered");
    coverageGatherer.getCoverageCriteria().stream()
                ...
                criterion
                    ...
                    .forEach(
                        (element, isCovered) ->
                            writeCSVRow(
                                path,
                                criterion.getType().toString()
                                    + ","
                                    + criterion.getRootPath()
                                    + ","
                                    + element
                                    + ","
                                    + isCovered)));

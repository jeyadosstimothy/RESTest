...
/**
 * Given an OpenAPI specification, obtain coverage level needed to reach 100% for each criterion
...
public class CoverageGatherer {
  ...
  /**
   * Creates all coverage criteria of the type passed. The function iterates over multiple elements
   * of the API (paths, operations, etc.) depending on the criterion type. For instance, for the
   * operations criterion, it iterates over all paths; for the parameter values criterion, it
   * iterates over all paths, operations of those paths and parameters of those operations.
   *
  ...
  private List<CoverageCriterion> getCoverageCriteria(CriterionType type) {
    ...
    if (type == PATH) {
      List<String> pathsList =
          new ArrayList<>(
              spec.getSpecification().getPaths().keySet()); // list of paths per criterion
  ...
  private void getOperationCoverageCriteria(CriterionType type, List<CoverageCriterion> criteria) {
    ...
    Iterator<Entry<String, PathItem>> pathsIterator =
        spec.getSpecification().getPaths().entrySet().iterator();
    while (pathsIterator.hasNext()) {
      ...
      if (type == OPERATION) {
        ...
        for (Entry<HttpMethod, Operation> operation :
            currentPathEntry.getValue().readOperationsMap().entrySet()) {
  ...
  private void getAnotherCoverageCriteria(
      CriterionType type,
      List<CoverageCriterion> criteria,
      Entry<String, PathItem> currentPathEntry) {
    // iterate over the operations of that path
    Iterator<Entry<HttpMethod, Operation>> operationsIterator =
        currentPathEntry.getValue().readOperationsMap().entrySet().iterator();
    while (operationsIterator.hasNext()) {
      ...
      if (type == PARAMETER) {
        getParameterCoverageCriteria(
            criteria, currentPathEntry, currentOperationEntry, requestBody);
        ...
        getResponseBodyPropertiesCoverageCriteria(
            criteria, currentPathEntry, currentOperationEntry);
      } else if (type == AUTHENTICATION) {
        // TODO: Remove
        //                            List<String> authenticationList = new ArrayList<>(); // list
        // of authentications per criterion
        //                            if (currentOperationEntry.getValue().getSecurity() != null) {
        // // there could be no 'security' property, so check it before
        //                                for (Map<String, List<String>> authenticationScheme :
        // currentOperationEntry.getValue().getSecurity()) {
        //
        // authenticationList.add(authenticationScheme.keySet().iterator().next()); // collect
        // authentications for this operation
        //                                }
        //                                criteria.add(createCriterion(authenticationList,
        // AUTHENTICATION, currentPathEntry.getKey() + "->" +
  ...
  private void getParameterCoverageCriteria(
      List<CoverageCriterion> criteria,
      Entry<String, PathItem> currentPathEntry,
      Entry<HttpMethod, Operation> currentOperationEntry,
    ...
    if (currentOperationEntry.getValue().getParameters() != null) {
      for (Parameter parameter :
          currentOperationEntry
              .getValue()
              .getParameters()) { // collect query, path and header parameters for this operation
    ...
    if (requestBody != null
        && requestBody.getContent().keySet().stream()
            .anyMatch(
                x ->
                    x.matches(
                        MEDIA_TYPE_APPLICATION_JSON_REGEX))) { // if request body is not null and
      // accepts application/json
      parametersList.add("body"); // add body parameter
    } else if (requestBody != null
        && (requestBody.getContent().containsKey(MEDIA_TYPE_APPLICATION_X_WWW_FORM_URLENCODED)
  ...
  private void getParameterValueCoverageCriteria(
      List<CoverageCriterion> criteria,
      Entry<String, PathItem> currentPathEntry,
      Entry<HttpMethod, Operation> currentOperationEntry) {
    // iterate over the parameters of that operation
    if (currentOperationEntry.getValue().getParameters() != null) {
      Iterator<Parameter> parametersIterator =
          currentOperationEntry.getValue().getParameters().iterator();

      while (parametersIterator.hasNext()) {
        Parameter currentParameter = parametersIterator.next();

        // this criterion only applies for header, query and path parameters
        if (currentParameter.getIn().equals("query")
            || currentParameter.getIn().equals("header")
            || currentParameter.getIn().equals("path")) {
          List<String> parameterValuesList = getSchemaValues(currentParameter.getSchema());
          if (!parameterValuesList
              .isEmpty()) { // only if the parameter has enum values or is a boolean
            criteria.add(
                createCriterion(
                    parameterValuesList,
                    PARAMETER_VALUE,
  ...
  private List<String> getSchemaValues(Schema schema) {
    ...
    if (schema instanceof ComposedSchema) {
      ...
      List<Schema> paramSchemas =
          paramSchema.getAnyOf() != null ? paramSchema.getAnyOf() : paramSchema.getOneOf();
      if (paramSchemas != null) paramSchemas.forEach(ps -> paramValues.addAll(getSchemaValues(ps)));
    }
  ...
  private List<String> getIndividualSchemaValues(Schema schema) {
    ...
    if (BOOLEAN_TYPE.equals(paramType)) return Arrays.asList("true", "false");
    else if (paramEnumValues != null) return paramEnumValues;

    return new ArrayList<>();
  }

  private void getFormDataParameterValues(
      List<CoverageCriterion> criteria,
      Entry<String, PathItem> currentPathEntry,
      Entry<HttpMethod, Operation> currentOperationEntry,
      RequestBody requestBody) {
  ...
  private void getInputContentTypeCoverageCriteria(
      List<CoverageCriterion> criteria,
      Entry<String, PathItem> currentPathEntry,
      Entry<HttpMethod, Operation> currentOperationEntry) {
    RequestBody requestBody = currentOperationEntry.getValue().getRequestBody();
    List<String> contentTypes =
        requestBody != null ? new ArrayList<>(requestBody.getContent().keySet()) : null;
    if (contentTypes != null) { // there could be no 'requestBody' property, so check it before
      criteria.add(
          createCriterion(
              new ArrayList<>(contentTypes),
              INPUT_CONTENT_TYPE,
              currentPathEntry.getKey() + "->" + currentOperationEntry.getKey().toString()));
    }
  }

  private void getOutputContentTypeCoverageCriteria(
      List<CoverageCriterion> criteria,
      Entry<String, PathItem> currentPathEntry,
      Entry<HttpMethod, Operation> currentOperationEntry) {
    ...
    List<String> contentTypes =
        response != null && response.getContent() != null
            ? new ArrayList<>(response.getContent().keySet())
            : null;
    if (contentTypes != null
        && !contentTypes
            .isEmpty()) { // there could be no 'apiResponse' or 'content' property, so check it
      // before
      criteria.add(
          createCriterion(
              new ArrayList<>(contentTypes),
  ...
  private void getStatusCodeClassCoverageCriteria(
      List<CoverageCriterion> criteria,
      Entry<String, PathItem> currentPathEntry,
      Entry<HttpMethod, Operation> currentOperationEntry) {
    List<String> statusCodeClassesList =
        new ArrayList<>(); // list of statusCodeClasses per criterion
    statusCodeClassesList.add(
        "2XX"); // it is assumed that all API operations should have a successful response
    ...
    criteria.add(
        createCriterion(
            statusCodeClassesList,
            STATUS_CODE_CLASS,
            currentPathEntry.getKey() + "->" + currentOperationEntry.getKey().toString()));
  }

  private void getStatusCodeCoverageCriteria(
      List<CoverageCriterion> criteria,
      Entry<String, PathItem> currentPathEntry,
      Entry<HttpMethod, Operation> currentOperationEntry) {
    criteria.add(
        createCriterion(
            new ArrayList<>(
                currentOperationEntry
                    .getValue()
                    .getResponses()
                    .keySet()), // list of status codes for that operation
            STATUS_CODE,
            currentPathEntry.getKey() + "->" + currentOperationEntry.getKey().toString()));
  }

  private void getResponseBodyPropertiesCoverageCriteria(
      List<CoverageCriterion> criteria,
      Entry<String, PathItem> currentPathEntry,
      Entry<HttpMethod, Operation> currentOperationEntry) {
    // iterate over the responses of that operation
    for (Entry<String, ApiResponse> currentResponseEntry :
        currentOperationEntry.getValue().getResponses().entrySet()) {

      // iterate over the media type responses of that ApiResponse
      if (currentResponseEntry.getValue().getContent() != null) {
        for (Entry<String, MediaType> currentMediaTypeEntry :
            currentResponseEntry.getValue().getContent().entrySet()) {
          Schema mediaTypeSchema = currentMediaTypeEntry.getValue().getSchema();

          if (mediaTypeSchema != null
              && currentMediaTypeEntry
                  .getKey()
                  .matches(
                      MEDIA_TYPE_APPLICATION_JSON_REGEX)) { // if the response actually returns a
            // body
            addResponseBodyPropertiesCriterion(
                mediaTypeSchema,
                criteria,
                currentPathEntry.getKey()
                    + "->"
                    ...
                    + "->" // note the final arrow, since new elements will be added to the rootPath
                );
            break;
          }
        }
  ...
  /**
   * Given an OpenAPI property (either a root response property or a sub-property), if it contains
   * some sub-properties (i.e. it is an object or an array of objects), add a new
   * RESPONSE_BODY_PROPERTIES criterion to the list of criteria passed in as an argument, updating
   * the baseRootPath according to the depth level of the sub-property. This function is to be
   * called recursively, so as to cover all sub-properties of an object.
   *
   * @param mediaTypeSchema OpenAPI property to check if it contains sub-properties to cover
   * @param criteria List of coverage criteria where to include the RESPONSE_BODY_PROPERTIES
   *     criteria
   ...
   */
  private void addResponseBodyPropertiesCriterion(
      Schema mediaTypeSchema, List<CoverageCriterion> criteria, String baseRootPath) {
    ...
    if (mediaTypeSchema instanceof ComposedSchema) {
      //            addResponseBodyPropertiesCriterion(((ComposedSchema)
      // mediaTypeSchema).getAnyOf().get(0), criteria, baseRootPath);
      // TODO: Handle anyOf, oneOf and allOf
      // TODO: Handle better when type == null, which seems to be with allOf
    } else {
      if (mediaTypeSchema.get$ref()
          != null) { // the response is an object and its schema is defined in the OpenAPI 'ref' tag
        currentResponseRef = mediaTypeSchema.get$ref();
        rootPathSuffix += "{"; // update rootPathSuffix
      } else if (mediaTypeSchema instanceof ArraySchema
          && "array".equals(mediaTypeSchema.getType())) { // the response is an array
        if (((ArraySchema) mediaTypeSchema).getItems().get$ref()
            != null) { // each item of the array has the schema of the OpenAPI 'ref' tag
          currentResponseRef = ((ArraySchema) mediaTypeSchema).getItems().get$ref();
          rootPathSuffix +=
              "[{"; // update rootPathSuffix to reflect depth level inside the response body
        }
      } else if (mediaTypeSchema.getProperties() != null
          && "object"
              .equals(
                  mediaTypeSchema
                      .getType())) { // the response is an object and its schema is defined right
        // after
        openApiProperties = mediaTypeSchema.getProperties();
        rootPathSuffix += "{"; // update rootPathSuffix
      }
      if (currentResponseRef
          != null) { // if the response body refers to a OpenAPI definition, get properties from
        // that object
        ...
        if (currentResponseRef.matches("/properties/.*")) {
          openApiProperties =
              spec.getSpecification()
                  .getComponents()
                  .getSchemas()
                  .get(currentResponseRef.replaceAll("/properties/.*", ""))
                  .getProperties();
          ...
          Schema propertiesSchema =
              spec.getSpecification().getComponents().getSchemas().get(currentResponseRef);
          if (propertiesSchema != null) openApiProperties = propertiesSchema.getProperties();
        }
      }

      if (openApiProperties
          != null) { // if there are properties to cover in this iteration, add new criterion
        baseRootPath +=
            rootPathSuffix; // update rootPath with the suffix, since a new criterion will be added
        criteria.add(
            createCriterion(
                new ArrayList<>(openApiProperties.keySet()),
                RESPONSE_BODY_PROPERTIES,
                baseRootPath));
        for (Entry<String, Schema> openApiProperty :
            openApiProperties.entrySet()) { // Recursively add criteria for each property
          addResponseBodyPropertiesCriterion(
              openApiProperty.getValue(),
              criteria,
              baseRootPath
                  + openApiProperty.getKey()); // update rootPath with the name of the property
        }
      }
    }
  }
  ...
  /**
   * Helper function to create a coverage criterion. Given a list of elements as strings, it
   * transforms it into a Map with Boolean values representing whether that element has been covered
   * or not. All are set to 'false' (no elements covered at the beginning).
   *
   ...
   * @param rootPath Root path of the criterion. This can vary, be composed of a different number of
   *     elements depending on the criterion type
   * @return CoverageCriterion with all fields set (type, rootPath and elements)
   */
  private CoverageCriterion createCriterion(
      List<String> elementsList, CriterionType type, String rootPath) {
        ...
        elementsList.stream()
            .collect(
                Collectors.toMap(
                    ...
                    e ->
                        Boolean
                            .FALSE)); // create Map whose keys are the elements and whose values are
    // 'false'
    criterion.setElements(elements);
    criterion.setRootPath(
        rootPath); // set rootPath for the criterion (this together with the TYPE conform a unique
    // ID)
    return criterion;
  }

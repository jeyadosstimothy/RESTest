...
/**
 * This class allows to populate a List of TestResult objects given the path of a CSV containing the
 * test results
 */
public class TestManager {
    ...
    for (List<String> csvRow : csvRows) {
      TestResult tr =
          new TestResult(
              ...
              csvRow.get(2),
              csvRow.get(3),
              Boolean.parseBoolean(csvRow.get(4)),
              csvRow.get(5));
      testResults.add(tr);
    }
    return testResults;
    ...
    for (int i = start; i < stop; i++) {
      TestResult tr =
          new TestResult(
              ...
              csvRows.get(i).get(2),
              csvRows.get(i).get(3),
              Boolean.parseBoolean(csvRows.get(i).get(4)),
              csvRows.get(i).get(5));
      testResults.add(tr);
    }
  ...
  public static TestResult getLastTestResult(String csvPath) {
    ...
    return new TestResult(
        csvRows.get(csvRows.size() - 1).get(0),
        csvRows.get(csvRows.size() - 1).get(1),
        csvRows.get(csvRows.size() - 1).get(2),
        csvRows.get(csvRows.size() - 1).get(3));
  }
  ...
  public static List<TestCase> getTestCases(String csvPath) {
    ...
    for (List<String> csvRow : csvRows) {
      TestCase tc =
          new TestCase(
              csvRow.get(0),
              Boolean.parseBoolean(csvRow.get(1)),
              csvRow.get(4),
              csvRow.get(5),
              HttpMethod.valueOf(csvRow.get(6)));
  ...
  private static Map<String, String> stringParamsToMap(String stringParameters) {
    ...
    if (stringParameters.equals("")) return parameters;
    ...
    try {
      ...
      for (String pair : pairs) {
        ...
        parameters.put(
            decode(keyValue[0], StandardCharsets.UTF_8.toString()),
            keyValue.length == 2 ? decode(keyValue[1], StandardCharsets.UTF_8.toString()) : "");
      }
    } catch (UnsupportedEncodingException e) {
      parameters.clear();
      LogManager.getLogger(TestManager.class.getName())
          .warn("Parameters of test case could not be decoded.", e);

...
public class OASAPIValidator {
  ...
  private OASAPIValidator(OpenAPISpecification spec) {
    ...
    // Test case validator:
    // Whitelist: Fix for swagger-validation library: formData parameters defined as string should
    // not
    ...
    this.validator =
        OpenApiInteractionValidator.createFor(spec.getPath()).withWhitelist(whitelist).build();
  }

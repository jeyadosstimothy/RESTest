...
/**
 * This class manages multiple API keys or auth headers for a single API. Given a JSON file
 * containing all API keys or auth headers, the AuthManager automatically selects one of them
 * randomly.<br>
 * <br>
 ...
 * </pre>
 *
 * <b>NOTE:</b> You have to specify the auth keys orderly, because the AuthManager will select them
 * in that way. In other words, if you have the following JSON:
 *
 * <pre>
 * {
 *     "apikey_1": [
 ...
 * </pre>
 *
 * the AuthManager will use
 *
 * <pre>apikey_1 = abc, apikey_2 = ghi</pre>
 *
 * or
 *
 * <pre>apikey_1 = def, apikey_2 = jkl</pre>
 *
 * as auth keys.<br>
 * <br>
 * <b>EVERY ARRAY OF AUTH KEYS MUST HAVE THE SAME SIZE.</b> If not, you will get an exception.
 */
public class AuthManager {
  ...
  private int counter;
  private int
      itCounter; /* It resets to zero when the algorithm obtains an auth key for every parameter.
    ...
    try {
      ...
      authProperties =
          objectMapper.readValue(
              readFile(authPath), new TypeReference<Map<String, List<String>>>() {});
    } catch (IOException e) {
    ...
    try {
      ...
      refreshHeaders =
          objectMapper.convertValue(
              oauthProperties.get(REFRESH_HEADERS), new TypeReference<Map<String, String>>() {});
  ...
  private void refreshTokenIfNeeded() {
    if (nowInSeconds() > expiration - EXPIRATION_THRESHOLD) refreshToken();
  }

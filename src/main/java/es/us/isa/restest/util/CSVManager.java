...
public class CSVManager {
  ...
  public static List<String> readValues(String path) {
    ...
    try {
      ...
      for (CSVRecord record : records) values.add(record.get(0));
  ...
  /**
   * Returns a list with the values of all rows (including header, if any) of the input CSV file.
   * Each row is a list of strings (one element per field)
   *
   * @param path The path of the CSV file
  ...
  public static List<List<String>> readCSV(String path, char delimiter) {
    ...
    try {
      ...
      for (CSVRecord record : records) {
        ...
        for (String field : record) currentRow.add(field);
        rows.add(currentRow);
      }
    } catch (IOException ex) {
      logger.error("Error parsing CSV file: {}", path);
  ...
  /**
   * Returns a list with the values of all rows of the input CSV file. Each row is a list of strings
   * (one element per field)
   *
   * @param path The path of the CSV file
   * @param includeFirstRow Whether to include first row of the CSV in the result or not. Useful for
   *     excluding header.
   * @return the values of all rows of the CSV file
   */
  public static List<List<String>> readCSV(String path, Boolean includeFirstRow) {
    List<List<String>> rows = readCSV(path, ',');
    if (!includeFirstRow) rows.remove(0);
    return rows;
  }

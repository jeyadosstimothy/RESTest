...
public class SchemaManager {
  ...
  */
  ...
  /**
   * No support for anyOf, oneOf. Possible support for allOf. <br>
   * <br>
   * Given a schema, it generates a duplicate with all properties resolved (i.e., without "ref"
   * attributes).
   */
  public static Schema<?> generateFullyResolvedSchema(Schema<?> schema, OpenAPI spec) {
    ...
    else fullyResolvedSchema = generateFullyResolvedObjectSchema(resolvedSchema, spec);
  ...
  /**
   * Given a schema, it returns the same schema if it is already resolved (i.e., contains properties
   * and has no ref attribute), or the resolved schema corresponding to the ref attribute.
   */
  public static Schema<?> resolveSchema(Schema<?> schema, OpenAPI spec) {
    ...
    while (resolvedSchema.get$ref() != null) {
      resolvedSchema =
          spec.getComponents()
              .getSchemas()
              .get(resolvedSchema.get$ref().replace("#/components/schemas/", ""));
    }
  ...
  private static Schema<?> resolveSchemaAndUpdateRefPath(Schema<?> schema, OpenAPI spec) {
    ...
    while (resolvedSchema.get$ref() != null) {
      ...
      if (!Pattern.compile("/" + schemaSubRef + "/|/" + schemaSubRef + "$")
          .matcher(currentRefPath)
          .find()) {

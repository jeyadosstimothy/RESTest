...
/**
 * Helper class which implements the Allure confidentiality filter. It basically censors the API
 * keys showed in the Allure report.
 *
...
public class AllureAuthManager {
  ...
  public static void confidentialityFilter(List<String> authProperties, String allurePath)
      throws IOException {
    File resultsDir = new File(allurePath);
    String[] allowedExtensions = {"html"};

    for (Iterator<File> it = FileUtils.iterateFiles(resultsDir, allowedExtensions, false);
        it.hasNext(); ) {
      File f = it.next();
      ...
      for (String authProperty : authProperties) {
        if (fileString != null) {
          fileString =
              fileString.replaceAll("(" + authProperty + "([=:]))([^<&'])*([<&'])", "$1CENSORED$4");
        }
      }

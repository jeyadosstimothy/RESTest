...
public class ARTestCaseGenerator extends ConstraintBasedTestCaseGenerator {
  ...
  public TestCase generateNextTestCase(Operation testOperation, String faultyReason)
    ...
    Pair<TestCase, Double> bestResult =
        Pair.with(generateTestCase(testOperation, faultyReason), .0);
    ...
    if (!testCases.isEmpty()) {
      for (int i = 0; i < numberOfCandidates - 1; i++) {
        ...
        if (tc != null) {
          ...
          if (minDistance > bestResult.getValue1()) bestResult = Pair.with(tc, minDistance);
        }
      }
    }
  ...
  private TestCase generateTestCase(Operation testOperation, String faultyReason)
      throws RESTestException {
    TestCase test;
    switch (faultyReason) {
      case "none":

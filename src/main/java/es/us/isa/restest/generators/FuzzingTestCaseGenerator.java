...
/**
 * This class implements a generator of fuzzing test cases. It uses a customizable dictionary to
 * obtain fuzzing parameters. Those parameters are classified by type (string, integer, number,
 * boolean).
 *
...
public class FuzzingTestCaseGenerator extends AbstractTestCaseGenerator {
  ...
  private final ITestDataGenerator
      commonFuzzingGenerator; // Random strings to be used for all parameters
  private final ObjectMapper objectMapper = new ObjectMapper();

  private static Logger logger = LogManager.getLogger(FuzzingTestCaseGenerator.class.getName());

  public FuzzingTestCaseGenerator(
    ...
    if (testOperation.getTestParameters() != null) {
      for (TestParameter testParam : testOperation.getTestParameters()) {
        if (!testParam.getIn().equals("body")) {
          ...
          if (param.getEnumValues() != null) fuzzingList.addAll(param.getEnumValues());
          ITestDataGenerator generator = new RandomInputValueIterator<>(fuzzingList);
          nominalGenerators.replace(
              Pair.with(testParam.getName(), testParam.getIn()),
        ...
        if (testParam.getWeight() == null || rand.nextFloat() <= testParam.getWeight()) {
          if (!testParam.getIn().equals("body")) {
            tc.addParameter(
                testParam,
                nominalGenerators
                    .get(Pair.with(testParam.getName(), testParam.getIn()))
                    .get(rand.nextInt(2))
                    .nextValueAsString());
          } else {
  ...
  private void generateFuzzingBody(TestCase tc, TestParameter testParam, Operation testOperation) {
    Map.Entry<String, MediaType> mediaTypeEntry =
        testOperation.getOpenApiOperation().getRequestBody().getContent().entrySet().stream()
    ...
    if (requestBody != null) {
      ...
      if ("array".equals(schema.getType())) node = objectMapper.createArrayNode();
      else node = objectMapper.createObjectNode();
  ...
  private void generateFuzzingBody(
      Schema schema, JsonNode rootNode, List<String> requiredProperties) {
    ...
    for (Object o : entries) {
      ...
      if (requiredProperties == null
          ...
          || (requiredProperties != null
              && requiredProperties.contains(entry.getKey())) // Req. property
  ...
  private JsonNode createValueNode(Schema schema) {
    ...
    if (schema.getEnum() != null) fuzzingList.addAll(schema.getEnum());

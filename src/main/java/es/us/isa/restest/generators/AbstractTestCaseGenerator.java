...
public abstract class AbstractTestCaseGenerator {
  ...
  protected Map<Pair<String, String>, List<ITestDataGenerator>>
      nominalGenerators; // Nominal test data generators (random, boundaryValue, fixedlist...)
  protected Map<Pair<String, String>, List<ITestDataGenerator>>
      faultyGenerators; // Faulty test data generators (random, boundaryValue, fixedlist...)
  protected AuthManager authManager; // For if multiple API keys are used for the API
  protected Float faultyRatio =
      0f; // Ratio (0-1) of faulty test cases to be generated on each operation. Defaults to 0.1
  protected int numberOfTests; // Number of test cases to be generated for each operation
  private int maxTriesPerTestCase =
      100; // Maximum number of tries for generating a random test case conforming the input OAS
  // schema.
  ...
  public AbstractTestCaseGenerator(
    ...
    if (authPath == null) authPath = conf.getAuth().getHeadersPath();
    if (authPath == null) authPath = conf.getAuth().getOauthPath();
    if (authPath != null && conf.getAuth().getOauthPath() != null)
      this.authManager = new AuthManager(authPath, true);
    else if (authPath != null) this.authManager = new AuthManager(authPath);
   ...
   *
   * <ol>
   *   <li>Each parameter in the testConf must exist in the OAS
   *   <li>Each required parameter in the OAS must exist in the testConf and it must have a weight
   *       of "null" or "1"
   * </ol>
   *
  ...
  private void preconditions(TestConfigurationObject conf) {
    ...
    for (Operation testOperation : conf.getTestConfiguration().getOperations()) {
      List<OpenAPIParameter> requiredParams =
          OpenAPISpecificationVisitor.getRequiredParametersFeatures(
              testOperation.getOpenApiOperation());
      ...
      if (testOperation.getTestParameters() != null) {
        ...
        for (TestParameter testParameter : testOperation.getTestParameters()) {
          ...
          if (requiredParams.contains(param)
              && testParameter.getWeight() != null
              && !testParameter.getWeight().equals(1F)) {
            throw new IllegalArgumentException(
                "Each required parameter in the OAS must exist in the testConf and it must have a"
                    + " weight of 'null' or '1'; parameter: "
                    + testParameter.getName()
                    + ", in: "
                    + testParameter.getIn()
                    + ", weight: "
  ...
  public Collection<TestCase> generate() throws RESTestException {
    ...
    for (Operation testOperation : conf.getTestConfiguration().getOperations()) {
      TestConfigurationFilter filter =
          filters.stream()
              .filter(x -> x.getPath().equalsIgnoreCase(testOperation.getTestPath()))
              .findFirst()
              .orElse(null);
      int filterIndex = -1;
      ...
      switch (testOperation.getMethod().toLowerCase()) {
        ...
        default:
          throw new RESTestException(
              "Methods other than GET, POST, PUT, PATCH and DELETE are not "
  ...
  /**
   * Generate the next test case and update the generation index. To be implemented on each
   * subclass. This method MUST call the {@link #checkTestCaseValidity(TestCase)} method before
      ...
      for (TestParameter confParam : testOperation.getTestParameters()) {
        if (confParam.getWeight() == null || rand.nextFloat() <= confParam.getWeight()) {
          ...
          if (generator instanceof ObjectPerturbator) {
            test.addParameter(
                confParam,
                ((ObjectPerturbator) generator)
                    .getRandomOriginalStringObject()); // Objects are not perturbated yet
            perturbation = true;
          } else if (generator instanceof BodyGenerator) {
            test.addParameter(
                confParam,
                ((BodyGenerator) generator)
                    .nextValueAsString(false)); // Objects are not mutated yet
          } else test.addParameter(confParam, generator.nextValueAsString());
        }
      }
    }

    // If a perturbation generator is included in the test configuration file, try to generate a new
    // (valid) test case by perturbating a valid input object
    if (perturbation) perturbate(test, testOperation);

    return test;
  }

  /**
   * Make sure the test case generated conforms to the specification. Otherwise, throw an exception
   * and stop the execution There's an exception: if stateful generators are configured we cannot
   * assure that the test case will be valid, therefore we omit this
   */
  protected void checkTestCaseValidity(TestCase test) throws RESTestException {
    if (!test.getFaulty() && checkTestCases && !hasStatefulGenerators) {
      ...
      if (!errors.isEmpty()) {
        throw new RESTestException(
            "The test case generated does not conform to the specification: " + errors);
      }
    }
  }
   ...
   */
  protected boolean makeTestCaseFaultyDueToInvalidGenerator(
      TestCase testCase, Operation testOperation) {

    if (faultyGenerators.isEmpty()) return false;
    ...
    Pair<String, String> faultyParam =
        (Pair<String, String>) possibleFaultyParams[rand.nextInt(possibleFaultyParams.length)];
    ...
    testCase.setFaultyReason(
        INDIVIDUAL_PARAMETER_CONSTRAINT
            + ":Set parameter "
            + faultyParam.getValue0()
            + " with invalid value "
            + faultyValue);
  ...
  /**
   * Tries to make a test case faulty by violating an individual constraint (ex. excluding a
   * required parameter)
   *
   * @param testCase The test case to modify
   * @param testOperation The operation the test case refers to
   ...
   * @throws RESTestException
   */
  protected boolean makeTestCaseFaultyDueToIndividualConstraints(
      TestCase testCase, Operation testOperation) throws RESTestException {
    ...
    if (testOperation.getOpenApiOperation().getRequestBody() != null) {
      TestParameter bodyParam =
          testOperation.getTestParameters().stream()
      ...
      if (bodyParam != null) {
        ...
        bodyGenerator =
            s.filter(x -> x instanceof BodyGenerator || x instanceof ObjectPerturbator)
                .findFirst()
                .orElse(null);
    ...
    while (i < generationAlternatives.size() && originalTest.equals(testCase)) {
      if (generationAlternatives
          .get(i)
          .equals("invalid_generator")) { // Test case with invalid generator
        makeTestCaseFaultyDueToInvalidGenerator(testCase, testOperation);
      }
      if (originalTest.equals(testCase)
          && generationAlternatives
              .get(i)
              .equals("request_body_mutation")) { // Test case with invalid body
      ...
      if (originalTest.equals(testCase)) { // Valid test case and mutate it
        String mutationDescription =
            TestCaseMutation.mutate(testCase, testOperation.getOpenApiOperation());
  ...
  /**
   * Tries to make a test case faulty by generating an invalid body. Works only if there's a body
   * and a BodyGenerator or ObjectPerturbator configured
   *
   * @param testCase The test case to modify
   * @param testOperation The operation the test case refers to
   ...
   * @throws RESTestException
   */
  protected boolean makeTestCaseFaultyDueToInvalidRequestBody(
      TestCase testCase, Operation testOperation, ITestDataGenerator bodyGenerator)
      throws RESTestException {
    TestCase originalTest = new TestCase(testCase);
    TestParameter bodyParam =
        testOperation.getTestParameters().stream()
    ...
    if (bodyGenerator != null && bodyParam != null) {
      ...
      // No invalid body generated. Return null and try to generate faulty test case in different
      // way
      if (errors.isEmpty()) {
        logger.warn(
            "Maximum number of tries reached when trying to generate invalid JSON body for the"
                + " operation {}",
    ...
    if (!originalTest.equals(testCase)) {
      ...
      testCase.setFaultyReason(
          INDIVIDUAL_PARAMETER_CONSTRAINT + ':' + " invalid request body: " + errorsMsg);
    }
  ...
  private void perturbate(TestCase test, Operation testOperation) {
    ...
    for (TestParameter confParam : testOperation.getTestParameters()) {
      ...
      if (generator instanceof ObjectPerturbator) {
        ...
        if (!valid) {
          test.addParameter(
              confParam, ((ObjectPerturbator) generator).getRandomOriginalStringObject());
          logger.warn(
              "Maximum number of tries reached when trying to perturbate the input object for the"
                  + " operation {}",
              testOperation.getOpenApiOperation().getOperationId());
        }
      }
        ...
        for (Generator g : param.getGenerators()) {
          ...
          if (gen instanceof ParameterGenerator) {
            ...
            ((ParameterGenerator) gen)
                .setParameterType(
                    OpenAPISpecificationVisitor.findParameterFeatures(
                            operation.getOpenApiOperation(), param.getName(), param.getIn())
                        .getType());
          }
  ...
  public void setNominalGenerators(
      Map<Pair<String, String>, List<ITestDataGenerator>> nominalGenerators) {
    this.nominalGenerators = nominalGenerators;
  }

  public void setFaultyGenerators(
      Map<Pair<String, String>, List<ITestDataGenerator>> faultyGenerators) {

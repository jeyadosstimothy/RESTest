...
public class RemoveRequiredParameter extends AbstractMutationOperator {
   ...
   * @param specOperation Swagger operation related to the test case. Necessary to extract all
   *     parameters that are subject to an invalid value change.
  ...
  public static String mutate(TestCase tc, Operation specOperation) {
    List<OpenAPIParameter> candidateParameters =
        getRequiredNotPathParametersFeatures(specOperation); // Path parameters cannot be removed
    ...
    OpenAPIParameter selectedParam =
        candidateParameters.get(ThreadLocalRandom.current().nextInt(0, candidateParameters.size()));

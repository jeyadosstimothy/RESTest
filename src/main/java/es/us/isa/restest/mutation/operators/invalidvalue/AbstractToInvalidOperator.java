...
/**
 * This class contains all possible mutations that can be performed to assign an invalid value to a
 * parameter. Specific mutation operators invoke this class' mutate() method to mutate a test case.
 *
...
public abstract class AbstractToInvalidOperator extends AbstractMutationOperator {
      ...
      switch (mutationsList.get(index)) {
        ...
        case REPLACE_WITH_BOOL:
          ...
          if (param.getEnumValues() == null || !param.getEnumValues().contains(randomBoolean)) {
            mutationApplied =
                getMutationMessage(mutationsList.get(index), param, tc, randomBoolean);
        ...
        case REPLACE_WITH_NUMBER:
          String randomNumber =
              Double.toString(ThreadLocalRandom.current().nextDouble(1000, 10001));
          ...
          if (param.getMax() != null) {
            ...
            if (param.getType().equals(NUMBER_TYPE))
              ...
              tc.addParameter(
                  param,
                  Integer.toString(
                      param.getMax().intValue() + ThreadLocalRandom.current().nextInt(1, 10)));
          }
          break;
          ...
          if (param.getMin() != null) {
            ...
            if (param.getType().equals(NUMBER_TYPE))
              tc.addParameter(
                  param,
                  Double.toString(
                      param.getMin().doubleValue()
                          - ThreadLocalRandom.current().nextDouble(1, 10)));
            else if (param.getType().equals(INTEGER_TYPE))
              tc.addParameter(
                  param,
                  Integer.toString(
                      param.getMin().intValue() - ThreadLocalRandom.current().nextInt(1, 10)));
          }
          break;
          ...
          if (param.getMaxLength() != null) {
            ...
            tc.addParameter(
                ...
                RandomStringUtils.randomAlphabetic(
                    param.getMaxLength() + ThreadLocalRandom.current().nextInt(1, 11)));
          }
          break;
  ...
  private static String getMutationMessage(
      String mutation, OpenAPIParameter param, TestCase tc, String newValue) {
    switch (mutation) {
      case REPLACE_WITH_INT:
        return "Changed value of "
            + param.getType()
            + (param.getEnumValues() != null ? " (enum) " : " ")
            + "parameter "
            + param.getName()
            + " from '"
            + tc.getParameterValue(param)
            + "' to integer '"
            + newValue
            + "'";
      case REPLACE_WITH_BOOL:

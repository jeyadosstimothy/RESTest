...
public class InvalidParameterValue extends AbstractMutationOperator {
  ...
  /**
   * If possible, inserts an invalid value into some parameter of a test case. For example, inserts
   * a string value into an integer parameter.
   *
   * @param tc Test case to mutate. NOTE: If the mutation is applied, the original {@code tc} object
   *     will not be preserved, it should be cloned before calling this method.
   * @param specOperation OpenAPI operation related to the test case. Necessary to extract all
   *     required parameters.
  ...
  public static String mutate(TestCase tc, Operation specOperation) {
    List<OpenAPIParameter> candidateParameters =
        getParametersFeaturesSubjectToInvalidValueChange(
            specOperation); // Parameters that can be mutated to create a faulty test case

    if (candidateParameters.isEmpty()) return "";

    OpenAPIParameter selectedParam =
        candidateParameters.get(
            ThreadLocalRandom.current()
                .nextInt(0, candidateParameters.size())); // Select one randomly

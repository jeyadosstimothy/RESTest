  ...
  public Object nextValue() {
    ...
    if (!values.isEmpty()) {
      if (minValues == 1 && maxValues == 1) {
        ...
        int numValues = 0;
        // while(minValues == null && d<1/2  (minValues == null || minValues > numValues) ||
        // ((maxValues == null || maxValues > numValues) && d<1/2)) {
  ...
  public String nextValueAsString() {
    ...
    if (nextValue instanceof List) {
      nextValueAsString =
          (String)
              ((List) nextValue)
                  .stream().map(x -> x.toString()).collect(Collectors.joining(separator));

...
public class RandomStringGenerator extends RandomGenerator {
  ...
  public String nextValue() {
    ...
    // Get string params
    int stringConf =
        4 * (includeAlphabetic ? 1 : 0)
            + 2 * (includeNumbers ? 1 : 0)
            + 1 * (includeSpecialCharacters ? 1 : 0);
      ...
      switch (stringConf) {
        ...
        default:
          throw new IllegalArgumentException(
              "Illegal stringConf for completeString method: " + stringConf);
      }
    } while (finalString.length()
        < minLength); // Repeat until minimum-length constraint is satisfied

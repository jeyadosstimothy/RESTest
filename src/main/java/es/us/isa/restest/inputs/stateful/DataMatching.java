...
/**
 * Utility class used by BodyGenerator and ParameterGenerator. Given a dictionary of values and a
 * parameter name, it extracts from the dictionary a potentially valid value for that parameter.
 */
public class DataMatching {

  private static final Morphology m = new Morphology();

  public static JsonNode getParameterValue(
      ObjectNode dict, String operationMethod, String operationPath, String paramName) {
    ...
    if ("id".equalsIgnoreCase(paramName)) {
      paramValue =
          getParameterValue(
              dict, operationMethod, operationPath, getIdParameterName(paramName, operationPath));
    ...
    if (paramValue == null) {
      ...
      while (dictIterator.hasNext())
        paramValues.addAll(
            getValuesOfSimilarParameterNames((ObjectNode) dictIterator.next(), paramName));
      if (!paramValues.isEmpty())
        paramValue = paramValues.get(ThreadLocalRandom.current().nextInt(paramValues.size()));
    }

    // 5th option: Repeat whole process with sub-property name (e.g., "data.comment.id" ->
    // "comment.id")
  ...
  private static List<JsonNode> getValuesOfSimilarParameterNames(
      ObjectNode operationDict, String paramName) {

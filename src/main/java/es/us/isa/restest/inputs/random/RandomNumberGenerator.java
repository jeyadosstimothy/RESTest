...
public class RandomNumberGenerator extends RandomGenerator {
  ...
  public RandomNumberGenerator(DataType type) {
    ...
    if (!type.isNumber()) throw new IllegalArgumentException("The requested type is not a number");

    this.type = type;

    // Set default min and max values
    if (type.equals(DataType.INTEGER)
        || type.equals(DataType.INT32)
        || type.equals(DataType.INT64)) {
  ...
  public Object nextValue() {
    ...
    } else if (type.equals(DataType.LONG)) value = (Long) rand.nextLong((Long) min, (Long) max);
    return value;
  }
  ...
  public String nextValueAsString() {
    ...
    else if (type.equals(DataType.FLOAT)) value = Float.toString((Float) nextValue());
    else if (type.equals(DataType.LONG)) value = Long.toString((Long) nextValue());
    return value;
  }

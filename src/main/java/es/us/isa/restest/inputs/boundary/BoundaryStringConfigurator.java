...
public class BoundaryStringConfigurator {
  ...
  public BoundaryStringConfigurator(
      ...
      int maxLength,
      int delta,
      boolean includeEmptyString,
      boolean includeNullCharacter) {
  ...
  public List<String> returnValues() {
    ...
    if (includeEmptyString) values.add("");
    if (includeNullCharacter) values.add("\0");
  ...
  private List<RandomStringGenerator> createRandomStringGenerators(
      int minLength, int maxLength, int delta) {
    ...
    strGens.add(
        new RandomStringGenerator(minLength + delta, minLength + delta, true, false, false));
    strGens.add(new RandomStringGenerator(maxLength, maxLength, true, false, false));
    strGens.add(
        new RandomStringGenerator(maxLength + delta, maxLength + delta, true, false, false));
    strGens.add(
        new RandomStringGenerator(
            (int) Math.ceil(((double) minLength + maxLength) / 2),
            (int) Math.ceil(((double) minLength + maxLength) / 2),
            true,
            false,
            false));
    ...
    if (minLength - delta > 0) {
      strGens.add(
          new RandomStringGenerator(minLength - delta, minLength - delta, true, false, false));
      strGens.add(
          new RandomStringGenerator(minLength - delta, minLength - delta, true, true, true));
    }
    if (maxLength - delta > 0) {
      strGens.add(
          new RandomStringGenerator(maxLength - delta, maxLength - delta, true, false, false));
      strGens.add(
          new RandomStringGenerator(maxLength - delta, maxLength - delta, true, true, true));
    }

...
public class BodyGenerator implements ITestDataGenerator {
  ...
  public JsonNode nextValue() {
    ...
    if (!checkIfExists(jsonPath) && defaultValue != null) {
      try {
        ...
        logger.warn(
            "The defaultValue used for the body of {}{} is not a valid JSON",
            operationMethod,
            operationPath);
      }
    }
    ...
    if (requestBody != null) {
      ...
      if ("array".equals(mutatedSchema.getType())) rootNode = objectMapper.createArrayNode();
      else rootNode = objectMapper.createObjectNode();
      try {
        generateStatefulObjectNode(dictNode, mutatedSchema, rootNode, "", new ArrayList<>(), true);
      } catch (RESTestException e) {
        logger.warn(
            "There isn't enough data to generate a valid request body for {} operation.",
  ...
  private void generateStatefulObjectNode(
      ObjectNode dictNode,
      Schema<?> schema,
      JsonNode rootNode,
      String prefix,
    ...
    String resolvedProperty =
        prefix
            .substring(prefix.lastIndexOf('.') + 1)
            .replace("-duplicated", "")
            .replace(DOT_CONVERSION, ".");

    if ("".equals(prefix) // First level object
        || requiredProperties == null
            && rootNode.isArray() // Array has no req. properties, but generate at least one
        || (requiredProperties != null
            && requiredProperties.contains(resolvedProperty)) // Req. property
        || ((requiredProperties == null || !requiredProperties.contains(resolvedProperty))
            && random.nextBoolean())) { // Optional property (50% prob.)
      JsonNode childNode = null;
      if (schema.getType() == null) {
        logger.warn(
            "The schema of {}{} has no type defined. RESTest will use a default value.",
            operationMethod,
            operationPath);
      }
      if (schema.getType().equals("object")) {
        ...
        if (schema.getProperties() != null) {
          for (Map.Entry<String, Schema> entry : schema.getProperties().entrySet()) {
            ...
            generateStatefulObjectNode(
                dictNode, entry.getValue(), childNode, newPrefix, schema.getRequired(), false);
          }
        }
      ...
      if (!"".equals(prefix) || !firstLevel) {
        if (childNode == null || childNode.isMissingNode()) {
          ...
          ((ObjectNode) rootNode)
              .set(
                  prefix.substring(prefix.lastIndexOf('.') + 1).replace(DOT_CONVERSION, "."),
                  childNode);
        } else {
        ...
        example =
            ((Example)
                    requestBody
                        .getExamples()
                        .values()
                        .toArray()[random.nextInt(requestBody.getExamples().values().size())])
                .getValue();
      ...
      if (example != null) {
        ...
        while (i < prefixSplit.length) {
          if (exampleNode.get(prefixSplit[i]) != null) {
            candidate =
                exampleNode.get(prefixSplit[i]).isArray()
                    ? exampleNode.get(prefixSplit[i]).get(0)
                    : exampleNode.get(prefixSplit[i]);
            exampleNode = exampleNode.get(prefixSplit[i]);
            i++;
          } else if (exampleNode.size() == 1
              && exampleNode.get(exampleNode.fieldNames().next()).isArray()) {

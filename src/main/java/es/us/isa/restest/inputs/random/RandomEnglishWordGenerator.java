...
public class RandomEnglishWordGenerator extends RandomGenerator {
  ...
  private final String[] LINKING_WORDS = {
    "the", "a", "and", "so", "for", "of", "hence", "thus", "if"
  };
  ...
  private static final Logger logger =
      LogManager.getLogger(RandomEnglishWordGenerator.class.getName());
  ...
  public String nextValue() {
    ...
    try {
      ...
      while (i < nWords) {
        ...
        // System.out.println("Number of words: " + nWords + " - Generated word: " +
        // generatedString.toString() + " (" + numberOfWords(generatedString.toString()) + ") -
        // Current random word: " + dictionaryEntry.getLemma() + " (" +
        // numberOfWords(dictionaryEntry.getLemma()) + ")");

        if ((numberOfWords(generatedString.toString()) + numberOfWords(dictionaryEntry.getLemma()))
            <= nWords) // Check it does not exceed the maximum number of words
        if (!excludeWords(
              dictionaryEntry.getLemma())) { // Check the word is not in the list of excluded words
            generatedString.append(dictionaryEntry.getLemma());
            i += numberOfWords(dictionaryEntry.getLemma());
            if (i < nWords) generatedString.append(" ");
          }
      }
  ...
  private boolean excludeWords(String sentence) {
    ...
    if (ignoreLinkindWords) {
      for (int i = 0; i < LINKING_WORDS.length && !result; i++)
        if (LINKING_WORDS[i].equalsIgnoreCase(sentence)) result = true;
    }

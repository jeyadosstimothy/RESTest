...
/**
 * This function maximizes the globalDiversity of a specific property of several elements. Only one
 * mode is possible:<br>
 * 1.- Diversity among inputs (HTTP requests).<br>
 * globalDiversity is computed as follows: 1) the number of unique pairs operationId-statusCode is
 * counted, and this number is added to globalDiversity; 2) for those pairs whose operationId is the
 * same, diversity (1 - similarity) is computed and added to globalDiversity; 3) globalDiversity is
    ...
    for (int i = 0; i < testCases.size(); i++) {
      ...
      for (int j = i + 1; j < testCases.size(); j++) {
        ...
        if (testCase_i.getOperationId().equals(testCase_j.getOperationId()))
          globalDiversity +=
              1
                  - similarityMeter.apply(
                      testCase_i.getFlatRepresentation(), testCase_j.getFlatRepresentation());
        else globalDiversity += 1; // Distinct operations count +1 each
      }
    }
    if (normalize) globalDiversity /= ((double) (testCases.size() * (testCases.size() - 1)) / 2);
      ...
      if (testCase_i.getOperationId().equals(testCase.getOperationId()))
        maxSimilarity =
            Math.max(
                maxSimilarity,
                similarityMeter.apply(
                    testCase.getFlatRepresentation(), testCase_i.getFlatRepresentation()));
    }

...
public class TestCase implements Serializable {
  ...
  private Boolean
      fulfillsDependencies; // True if it does not violate any inter-parameter dependencies
  private String
      faultyReason; // "none", "individual_parameter_constraint", "invalid_request_body" or
  // "inter_parameter_dependency"
  private Boolean
      enableOracles; // True (default) if you want to assert the response. False if you only want to
  // execute the request
  ...
  public TestCase(String id, Boolean faulty, String operationId, String path, HttpMethod method) {
    this.id = id;
    this.faulty = faulty;
    this.fulfillsDependencies =
        false; // By default, a test case does not satisfy inter-parameter dependencies
  ...
  private String processPathParameter(String pathParamValue) {
    if ("".equals(pathParamValue)) return "null";
  ...
  public String getFlatRepresentation() {
    ...
    List<String> queryParameters =
        new ArrayList<>(this.getQueryParameters().keySet()); // Query parameters
    ...
    List<String> headerParameters =
        new ArrayList<>(this.getHeaderParameters().keySet()); // Header parameters
    Collections.sort(headerParameters);
    for (String headerParameter : headerParameters)
      tcRepresentation
          .append(headerParameter)
          .append(this.getHeaderParameters().get(headerParameter));

    List<String> formDataParameters =
        new ArrayList<>(this.getFormParameters().keySet()); // FormData parameters
    Collections.sort(formDataParameters);
    for (String formDataParameter : formDataParameters)
      tcRepresentation
          .append(formDataParameter)
          .append(this.getFormParameters().get(formDataParameter));
  ...
  public void exportToCSV(String filePath) {
    ...
    try {
      for (Map.Entry<String, String> h : headerParameters.entrySet()) {
        rowEnding
            .append(encode(h.getKey(), StandardCharsets.UTF_8.toString()))
            .append("=")
            .append(encode(h.getValue(), StandardCharsets.UTF_8.toString()))
            .append(";");
      }
      rowEnding.append(",");
      for (Map.Entry<String, String> p : pathParameters.entrySet()) {
        rowEnding
            .append(encode(p.getKey(), StandardCharsets.UTF_8.toString()))
            .append("=")
            .append(encode(p.getValue(), StandardCharsets.UTF_8.toString()))
            .append(";");
      }
      rowEnding.append(",");
      for (Map.Entry<String, String> q : queryParameters.entrySet()) {
        rowEnding
            .append(encode(q.getKey(), StandardCharsets.UTF_8.toString()))
            .append("=")
            .append(encode(q.getValue(), StandardCharsets.UTF_8.toString()))
            .append(";");
      }
      rowEnding.append(",");
      for (Map.Entry<String, String> f : formParameters.entrySet()) {
        rowEnding
            .append(encode(f.getKey(), StandardCharsets.UTF_8.toString()))
            .append("=")
            .append(encode(f.getValue(), StandardCharsets.UTF_8.toString()))
            .append(";");
      }
  ...
  public List<String> getValidationErrors(OpenApiInteractionValidator validator) {
    ...
    SimpleRequest.Builder requestBuilder =
        new SimpleRequest.Builder(this.getMethod().toString(), fullPath)
    ...
    if (this.getFormParameters().size() > 0) {
      ...
      try {
        for (Map.Entry<String, String> formParam : this.getFormParameters().entrySet()) {
          formDataBody
              .append(encode(formParam.getKey(), StandardCharsets.UTF_8.toString()))
              .append("=")
              .append(encode(formParam.getValue(), StandardCharsets.UTF_8.toString()))
              .append("&");
        }
      } catch (UnsupportedEncodingException e) {
  ...
  public static Boolean checkFulfillsDependencies(TestCase tc, Analyzer idlReasoner) {
    if (idlReasoner == null) return true;
    try {
      return idlReasoner.isValidRequest(
          restest2idlTestCase(tc)); // Previous version of IDLReasoner:
      // idlReasoner.isValidRequest(restest2idlTestCase(tc), true);
    } catch (IDLException e) {
      logger.warn(
          "There was an error generating an invalid request with IDLReasoner: {}", e.getMessage());
      return false;
    }
  }
  ...
  public int hashCode() {
    ...
    result =
        prime * result + ((fulfillsDependencies == null) ? 0 : fulfillsDependencies.hashCode());
